...
| **v019** | **goal_brief** | **goals.+goal_brief_json（Final Goal Canonical Brief 七字段 JSON；历史零改动）** |
...
| **DEV-0055** | **First-Layer Productization & Planning Truth** | **Truth Map 全量审计 + UI Constitution + Canonical Final Goal（v019 goal_brief+冲突不自动选+Readiness 门）+ Dedicated Planning Pipeline（intent→clarify→PlanDraft→Validator[层级/超载/重复/粒度]→Deterministic Compiler→ChangeSet；14 天滚动；修复作文无 ChangeSet）+ Today 第一层（减法/直接动作/无分类 chips）+ Completion 即时反馈 + /data 一级页（Allowlist 六块/后端聚合/recharts route-lazy）+ Planning 减肥 + AI Panel 默认收起 + Knowledge startSession 守卫** | **v019 一列；Direct Write Tools 仍 0；+recharts+@radix-ui/themes** | **v019** | ✅ Gate 277tests（实机 v019 首启待确认） |
