...
| 字段 | 值 |
|---|---|
| Context Version | **HGCTX-0002**（DEV-0057 完成晋升） |
| Current Schema | **v020**（20 migrations：…v019 goal_brief / v020 goal_truth_convergence=Canonical Title 修复 + duration_review_state） |
| Last Completed DEV | **DEV-0057**（Core Reliability, Data Trust & Architecture Convergence；前序：DEV-0056 → 本 Context 制度建立） |
| Current DEV | 无（Idle / Awaiting Next TASK） |
| Last Updated | 2026-08-17（DEV-0057 完成） |
| Source Fingerprint | Git：main @ 457fe5e；**WORKTREE DIRTY：YES**（总 2681 项=2555 webview 噪声 staged-D[已 gitignore+untrack，待用户 commit] + ~126 实际改动[DEV-0052→0057 全部未提交工作]；HEAD≠当前代码，以工作区为准） |
| Runtime Status | 实机：v018 曾确认；**v019/v020 实机首启 NOT VERIFIED**（v020 含数据修复迁移，首启将自动 backup-first 逻辑见 §Runtime Integrity） |
| Gate Status | tsc 0 / npm run build 通过(546ms) / cargo check **0 err 0 warn** / cargo test **293 passed · 0 failed · 0 SAC**（27+1 套件；batch056 新 16） / tauri dev：本轮未执行（历轮 SAC 4551） |
| Document Status | 本文件 CURRENT；`audit/`=审计时点证据（非永久当前）；`history/`=仅历史；PROJECT.md=DEPRECATED 指针 |
...
| `PROJECT.md` | Deprecated 指针 | NONE | 冻结 |
| `archive/history/*` | 历史证据（含 8 份 ai-operations、旧 ENV/PROJECT 快照、PROJECT.md 最终版） | HISTORICAL ONLY | 只增不删 |
| `tmp/` | 临时区（DB 副本/探测脚本） | NONE | — |
...
## 1. Executive Summary
**一句话（源码核对后保留审计定义）**：Higher 是一个「以学习事件（StudySession）为唯一事实、双树（Goal×Knowledge）组织方向、AI 经审批管线辅助规划、全部数据本地」的个人学习桌面系统（Tauri 2 + React 19 + SQLite v019）。
...
## 6. Database Snapshot
- Schema **v020** / 20 migrations / dev DB=`src-tauri/.data/higher.db` / prod=`%LOCALAPPDATA%\com.higher.desktop\higher.db`；**Connection Model = 单 SQLite 连接 `DbState(Mutex)`**（长事务会阻塞 IPC——已知架构风险）。
- v020 变更：①Canonical Title 修复（brief.title 非空≠name → name 同步，幂等）；②study_sessions +`duration_review_state`(normal|needs_review|confirmed|corrected) + 存量回填（>12h 未审核→needs_review；time_corrected→corrected）。
- 表分类：核心业务 8（profiles/goals/tasks/sessions/learning_items/knowledge_documents/evaluations/recurring_rules）· 反馈主线 2（feedbacks/adjustments）· AI 全家族 7（conversations/messages/runs/sources/memory/personalization×3/change_sets+operations）· 基础设施 4（search_index+FTS5/settings/attachments）。完整字段/FK/索引 → `archive/audit/HIGHER_DATA_MODEL.md`（v019 时点+v020 增量见本节）。
...
|---|---|---|---|---|---|---|---|
| DEV-0053 | 08-16 | AI 真实性三层防线/Today 两区/日报/双树快照/Ref 规划 | daily_report/planner 前身 | v018 | Today/Calendar/AI 治理 | 无已知 | 实机 v018 ✅ |
...
# Active Development
- Task：**无**（DEV-0057 已完成并晋升）
- Status：**COMPLETED / IDLE**（Awaiting Next TASK）

# In-Progress Delta
（空——Gate 通过后全部已验证变化提升入 Confirmed Current State；详见 TRAE_RUN DEV-0057）

# Human Runtime Checklist（DEV-0057 · 仅无法自动验证项 · ≤8）
1. **v019→v020 实机升级**：启动看迁移日志（`applied v020 goal_truth_convergence`）；若有已填 brief.title 的目标，Planning 目标卡名称应与 title 一致
2. **创建/编辑 Final Goal**：保存 brief 后树根名称同步为 title
3. **Active 冲突弹窗三入口**：学习中先点 Today 快速学习 → 再试 知识页开始学习 / 任务开始（三个入口都应弹「当前已有学习进行中」）
4. **一条超长 Session**：结束一个 >12h 会话（可把 started_at 手动改早）→ Completion 显示「学习已结束+请确认」而非祝贺；确认/修正后 Data/日报统计恢复计入
5. **Data 统计变化**：累计/日均为 `XhYm` 格式；0 分钟知识节点不显示；有待确认记录时顶部出现提示
6. **真实 AI Key 下 14 天 Planner**：「帮我安排未来14天并加入Higher」→（目标不完整先补全）→ 生成计划审查卡 → 应用 → Today/Calendar 真实出现
7. **ChangeSet 审查+Apply**：含目标+知识混合的大方案的分组显示；应用后 ✓ 行；「撤销」后 final 仍在
8. **媒体播放**：学习笔记中的图片/视频正常显示（asset 主路径）；大视频不再卡顿；Settings→审计与备份 可创建快照（prod 安装版验证快照 size>0）
