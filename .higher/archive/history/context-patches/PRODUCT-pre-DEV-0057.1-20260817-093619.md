...
> Last Verified：2026-08-17 00:40（DEV-0055）
> Schema：v019
...
- **四层产品模型**：记录（我今天做了什么）→ 专注（这段怎么进行）→ 知识（时间形成了什么）→ 智能（下一步怎么调整）；当前重点 Layer 1
- **Canonical Final Goal**：Final Goal = Planning 唯一目标事实源（goal_brief：title/outcome/deadline/success_criteria/scope/constraints/unresolved）；Profile 名称≠目标；多源冲突必须用户确认（不自动选）
- **AI Planning Pipeline**：规划写意图→Goal Readiness（缺项≤5 问澄清）→结构化 PlanDraft→Backend 验证（层级/超载/重复）→确定性编译→ChangeSet→用户审批；默认 14 天滚动；AI 只说「已准备好计划」
- **Evidence-based Feedback**：学习完成即时反馈（时长/今天累计/任务进度/知识归属）；所有爽感可追溯到真实学习证据
- **/data 一级页**：学习天数/累计/日均+今日+趋势+Knowledge 时间分布（可下钻）+学习时段+计划vs实际；严格 Allowlist（无评分无 Session 计数）；无数据=有意义空态
- **Today=我现在要做什么**：任务（开始/查看）+活动（打开/整理）；高频直接可见；无默认分类统计
- **UI Constitution**：.higher/UI_CONSTITUTION.md（20 条永久规则+数据展示四问）
