...
| 字段 | 值 |
|---|---|
| Context Version | **HGCTX-0019**（DEV-0065.4 v1.0.0 Release Freeze：AUTOMATED RELEASE GATE PASSED · HUMAN RUNTIME PENDING） |
| Active Development | **DEV-0065.4**（版本冻结 1.0.0 / WebView2 轻量化 / mock 发布隔离 / RC 构建；零 schema · 零依赖 · 零产品行为变更） |
| Release Line | **release/1.0.0** @ baseline `bde225d`（v1 预备基线）；Target Version **1.0.0**（六处对齐：tauri.conf/package/package-lock×2/Cargo.toml/Cargo.lock）；**Tag: NOT CREATED · GitHub Release: NOT CREATED · Human Runtime: PENDING** |
...
| Windows Release | **v1.0.0 轻量 NSIS**：mainBinaryName=Higher.exe · currentUser · SimpChinese · displayLanguageSelector=false · startMenuFolder=Higher · installerIcon/uninstallerIcon=icons/icon.ico · targets=nsis only · useLocalToolsDir=true；**WebView2 = downloadBootstrapper（v1.0.0 决策，替换 0.3.0 的 offlineInstaller；离线 WebView2 载荷不再内嵌）**；构建入口 Build-Higher-Release.ps1（HIGHER_RELEASE_BUILD=1 排除 public/ + dist 双阶段卫生门 + 120 MiB 尺寸预算 + SHA256 独立复验 + finally 恢复 env）；RC 产物 `release\Higher_1.0.0_Setup.exe` **6,025,655 bytes（5.75 MiB）** + SHA256.txt（f727665b…b6bd0，复验 match）；前值 0.3.0 为 221,745,349 bytes（↓97.3%） |
...
| Schema | **v024**（0 migration；Backend Domain 0；AI Runtime 0；Dependency 0；DEV-0065.4 起版本六处对齐 **1.0.0**：tauri.conf/package/package-lock×2/Cargo.toml/Cargo.lock + README） |
| Last Updated | 2026-08-23（DEV-0065.4 收口，系统时间） |
| Source Fingerprint | Git：**release/1.0.0** @ **bde225d**（v1 预备基线已提交；65.4 freeze 变更未提交）；**WORKTREE DIRTY：DEV-0065.4 未提交**（变更=版本六处/WebView2/mock 隔离 index+vite/构建脚本/README/两测试文件/ENV+TRAE_RUN；**Product Source Delta = release-only index.html+vite.config.ts**，§24 冻结目录零 diff） |
| Runtime Status | b14e237=stable AI runtime；65.2R 发布已入库（005006e）；**DEV-0065.4：RC 安装包已构建（dirty worktree，非最终 GitHub 产物）——正式发布需：commit → clean rebuild → Human Runtime → tag v1.0.0 → GitHub Release**；生产 AppLocalData 未触碰 |
| Gate Status | **DEV-0065.4 recorded automated release gate**：tsc **0 errors** / npm build **通过（release 模式 dist/mock=absent）** / cargo check **0 errors**（Cargo.lock 仅 1 行版本 diff）/ **batch0654_release_freeze 11/11**（V01-V11 发布不变量）/ batch0652_release **20/20**（r02→1.0.0、r07→downloadBootstrapper、r19 语义收窄，§29 授权）/ batch0651_ui **20/20** / batch064r2_ui **27/27** / batch064_ui **28/28** / batch063_ui **18/18** / ai_panel **8/8** / batch062r1 **41/41** / batch062r **44/44** / batch062 **57/57**（串行；合计 274）；**§43 secret 扫描零命中 · §44 dev data 零 tracked · dist= index.html+assets（14 文件 3,131,381 B，零 .map/.ts/.tsx/.rs）** |
