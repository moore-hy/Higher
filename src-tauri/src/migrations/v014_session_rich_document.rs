...
pub fn up(conn: &rusqlite::Connection) -> rusqlite::Result<()> {
    ...
    let mut stmt = conn.prepare(
        "SELECT COUNT(*) FROM pragma_table_info('study_sessions') WHERE name='note_document_json'",
    )?;
    let exists: i64 = stmt.query_row([], |r| r.get(0))?;
    if exists == 0 {
        conn.execute_batch("ALTER TABLE study_sessions ADD COLUMN note_document_json TEXT;")?;
    }
    Ok(())
}
