...
use crate::companion::types::{
    ExpeditionReadiness, THEME_ELECTRONICS, THEME_ENGLISH, THEME_GENERAL, THEME_MATH,
    THEME_PROGRAMMING, THEMES,
};
...
const THEME_KEYWORDS: [(&str, &[&str]); 4] = [
    (
        THEME_ENGLISH,
        &[
            "english", "英语", "英文", "单词", "词汇", "vocab", "grammar", "语法", "听力",
            "listening", "翻译", "translation", "reading", "阅读", "toefl", "ielts", "cet",
            "四级", "六级", "专四", "专八",
        ],
    ),
    (
        THEME_MATH,
        &[
            "math", "数学", "代数", "algebra", "几何", "geometry", "微积分", "calculus",
            "概率", "统计", "statistics", "probability", "线性代数", "矩阵",
        ],
    ),
    (
        THEME_PROGRAMMING,
        &[
            "编程", "代码", "programming", "code", "rust", "python", "java", "javascript",
            "typescript", "算法", "algorithm", "数据结构", "git", "sql", "编译器",
        ],
    ),
    (
        THEME_ELECTRONICS,
        &[
            "电子", "electronics", "电路", "circuit", "模电", "数电", "单片机", "mcu",
            "fpga", "信号", "放大器", "电阻", "电容", "verilog", "spice", "编码器",
        ],
    ),
];
...
pub fn normalize_theme(raw: &str) -> &'static str {
    THEMES.iter().find(|t| **t == raw).copied().unwrap_or(THEME_GENERAL)
}
...
mod unit_tests {
    ...
    fn readiness_thresholds_are_locked() {
        assert_eq!(readiness_from_contribution(0), ExpeditionReadiness::NotReady);
        assert_eq!(readiness_from_contribution(4), ExpeditionReadiness::NotReady);
        assert_eq!(readiness_from_contribution(5), ExpeditionReadiness::ReadyShort);
        assert_eq!(readiness_from_contribution(14), ExpeditionReadiness::ReadyShort);
        assert_eq!(readiness_from_contribution(15), ExpeditionReadiness::ReadyMedium);
        assert_eq!(readiness_from_contribution(29), ExpeditionReadiness::ReadyMedium);
        assert_eq!(readiness_from_contribution(30), ExpeditionReadiness::ReadyLong);
        assert_eq!(readiness_from_contribution(40), ExpeditionReadiness::ReadyLong);
    }
    ...
    fn readiness_availability_is_locked() {
        assert!(ExpeditionReadiness::NotReady.available_durations().is_empty());
        assert_eq!(
    ...
    fn theme_inference_is_conservative() {
        ...
        assert_eq!(
            infer_theme(&["英语单词".to_string(), "听力训练".to_string(), "数学".to_string()]),
            THEME_ENGLISH
        );
    }
