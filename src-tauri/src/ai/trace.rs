...
impl Trace {
    ...
    fn emit(&self, conn: &Connection, event: &str, mut data: serde_json::Value) {
        if let Some(o) = data.as_object_mut() {
            o.insert(
                "t_ms".into(),
                json!(self.started.elapsed().as_millis() as i64),
            );
        }
        let _ = conn.execute(
    ...
    pub fn route_decided(&self, conn: &Connection, route: &str, how: &str, skills: &[String]) {
        self.emit(
            conn,
            "route_decided",
            json!({ "route": route, "decided_by": how, "skill_ids": skills }),
        );
    }
    ...
    pub fn turn_decided(&self, conn: &Connection, decision: &str, how: &str) {
        self.emit(
            conn,
            "turn_decided",
            json!({ "decision": decision, "decided_by": how }),
        );
    }
    ...
    pub fn context_built(&self, conn: &Connection, context_chars: usize, chips: &[String]) {
        self.emit(
            conn,
            "context_built",
            json!({ "context_chars": context_chars, "chips": chips }),
        );
    }

    pub fn provider_request_started(
        &self,
        conn: &Connection,
        index: i64,
        kind: &str,
    ...
    pub fn provider_first_delta(&mut self, conn: &Connection, index: i64) {
        if !self.first_delta_recorded {
            ...
            self.emit(
                ...
                json!({ "provider_request_index": index }),
            );
        }
    }
    ...
    pub fn tool_round(&self, conn: &Connection, round: usize, tool_count: usize) {
        self.emit(
            conn,
            "tool_round_started",
            json!({ "tool_round": round, "tool_count": tool_count }),
        );
    ...
    pub fn action_plan_compiled(&self, conn: &Connection, operation_count: usize) {
        self.emit(
            ...
            json!({ "operation_count": operation_count }),
        );
    }
