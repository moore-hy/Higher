...
fn seed_legacy_tasks(conn: &Connection, p: i64) {
    ...
    .enumerate()
    {
        ...
        seed_task(
            conn,
            p,
            &format!("高数第{n}章+英语单词200词+408数据结构结论（旧复合{n}）"),
            d,
            "pending",
        );
    }
...
fn two_turn_e2e(name: &str, with_memory: bool) -> (DbState, VaultState, i64, i64) {
    ...
    let out2 = run_turn(
        ...
        vec![
            t2_full_answer_tool(),
            text_completion(&new_plan_draft().to_string()),
        ],
    )
    .unwrap();
    assert_eq!(
        out2, "completed",
        "E2E T2：同 Turn 自动继续并收口（禁 Turn 3）"
    );
    (state, vault, pid, cid)
}
...
fn f2_tc002_full_answer_auto_continue() {
    ...
    assert_eq!(
        ...
        "TC002/TC015：Replacement → 现有正式确认语义"
    );
    let n: i64 = conn
        .query_row(
    ...
    assert_eq!(
        n, 1,
        "TC013/TC017：ONE ChangeSet（Repair/Replacement 不另开包）"
    );
    ...
    assert_eq!(
        archived, 0,
        "TC015：确认前 0 business mutation（旧任务未动）"
    );
    let new_n: i64 = conn
        .query_row(
    ...
    assert_eq!(
        new_n, 0,
        "TC015：确认前新任务未落库（整体生效，无 partial apply）"
    );
...
fn f2_tc008_replacement_window_selector() {
    ...
    assert_eq!(
        selected.len(),
        6,
        "TC008：只选中 5 复合 + 1 meta（{titles:?}）"
    );
    assert!(
        titles.iter().all(|t| t.contains("旧")),
        "TC008：选中者均为旧未来任务"
    );
    for forbidden in [
        "窗口内已完成任务",
...
fn f2_tc011_new_tasks_atomic() {
    ...
    for t in new_titles {
        let n: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM tasks WHERE profile_id=?1 AND title=?2",
                params![pid, t],
                |r| r.get(0),
            )
            .unwrap();
...
fn f2_tc017_idempotency() {
    let (state, vault, pid, _cid) = two_turn_e2e("tc017", false);
    {
        ...
        assert_eq!(
            count(&conn, "ai_change_sets"),
            1,
            "TC017：single effective proposal"
        );
    }
...
fn fallback_guard_blocks_generic_on_active_continuation() {
    ...
    assert_eq!(
        out2, "failed",
        "Guard：planning continuation 不完整 → run failed（不假装 completed）"
    );
    let conn = state.0.lock().unwrap();
    let err: String = conn
        .query_row(
            "SELECT COALESCE(error,'') FROM ai_runs WHERE id='g2'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(
        err, "planning_continuation_incomplete",
        "Guard：durable 错误码"
    );
    let reply = last_assistant(&conn, cid, pid);
    assert!(
...
fn replacement_intent_detection() {
    ...
    assert!(
        !is_replacement_intent("帮我规划未来14天数学英语408学习计划"),
        "普通规划请求 ≠ 替换意图"
    );
