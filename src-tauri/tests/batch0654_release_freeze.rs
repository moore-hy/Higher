...
fn v01_version_100_everywhere() {
    ...
    assert_eq!(
        ...
        "V01: package-lock packages[\"\"] = 1.0.0"
    );
    assert_eq!(
        cargo_pkg_version(&read_manifest("Cargo.toml")),
        "1.0.0",
        "V01: Cargo.toml = 1.0.0"
    );
    assert_eq!(
        cargo_lock_app_version(&read_manifest("Cargo.lock")),
        "1.0.0",
        "V01: Cargo.lock app = 1.0.0"
    );
}
...
fn v02_identity_frozen() {
    ...
    assert_eq!(
        c["identifier"], "com.higher.desktop",
        "V02: identifier 冻结"
    );
    assert_eq!(
        c["mainBinaryName"], "Higher",
        "V02: mainBinaryName = Higher"
    );
    for banned in [
        "Higher1",
        "Higher-v1",
...
fn v03_bundle_contract() {
    ...
    assert!(targets.iter().any(|t| t == "nsis"), "V03: targets 含 nsis");
    assert!(!targets.iter().any(|t| t == "msi"), "V03: 无 MSI");
    assert_eq!(
        c["bundle"]["useLocalToolsDir"], true,
        "V03: useLocalToolsDir = true"
    );
    assert_eq!(
        c["build"]["frontendDist"], "../dist",
        "V03: frontendDist = ../dist"
    );
    assert_eq!(
        c["app"]["windows"],
        serde_json::json!([]),
        "V03: app.windows 空（动态主窗口）"
    );
}
...
fn v04_nsis_full_contract() {
    ...
    assert_eq!(
        n["installMode"], "currentUser",
        "V04: installMode = currentUser"
    );
    let langs = n["languages"].as_array().expect("languages");
    assert!(langs.iter().any(|l| l == "SimpChinese"), "V04: SimpChinese");
    assert_eq!(
        n["displayLanguageSelector"], false,
        "V04: displayLanguageSelector = false"
    );
    assert_eq!(
        n["startMenuFolder"], "Higher",
        "V04: startMenuFolder = Higher"
    );
    assert_eq!(n["installerIcon"], "icons/icon.ico", "V04: installerIcon");
    assert_eq!(
        n["uninstallerIcon"], "icons/icon.ico",
        "V04: uninstallerIcon"
    );
}
...
fn v05_webview2_download_bootstrapper() {
    ...
    assert_eq!(
        ...
        "V05: v1.0.0 = downloadBootstrapper"
    );
    for banned in [
        "offlineInstaller",
        "fixedRuntime",
        ...
        assert!(wv["type"] != banned, "V05: 官方发布不得使用 {banned}");
    }
}
...
fn v08_build_script_sets_release_env() {
    ...
    assert!(
        s.contains("HIGHER_RELEASE_BUILD"),
        "V08: 脚本设置 HIGHER_RELEASE_BUILD"
    );
    assert!(s.contains("'1'") || s.contains("\"1\""), "V08: 设置为 1");
    // finally 恢复语义
    assert!(s.contains("finally"), "V08: finally 恢复环境变量");
    assert!(
        s.contains("Remove-Item Env:\\HIGHER_RELEASE_BUILD"),
        "V08: 原值不存在时移除"
    );
}
...
fn v09_no_raw_bundle_resources() {
    ...
    assert!(
        !raw.contains("\"resources\""),
        "V09: 无 bundle resources 原始映射"
    );
...
fn v11_build_script_contract() {
    ...
    assert!(
        s.contains("tauri.conf.json"),
        "V11: 动态读取 tauri.conf.json"
    );
    assert!(
        s.contains("$Version = $Conf.version"),
        "V11: 版本来自 conf.version"
    );
    // 产物命名
    assert!(
        s.contains("Higher_${Version}_Setup.exe"),
        "V11: Higher_<version>_Setup.exe"
    );
    assert!(
        s.contains("Higher_${Version}_SHA256.txt"),
        "V11: Higher_<version>_SHA256.txt"
    );
    // worktree 干净门
    assert!(
        s.contains("git status --porcelain"),
        "V11: clean-worktree guard"
    );
    // dist 卫生门
    assert!(
        s.contains("RELEASE_FRONTEND_CONTAMINATED"),
        "V11: release frontend hygiene gate"
    );
    // 尺寸预算（120 MiB 硬上限）
    assert!(s.contains("125829120"), "V11: installer size ceiling");
    assert!(
        s.contains("INSTALLER_SIZE_BUDGET_MISS"),
        "V11: 尺寸超限 STOP"
    );
    // SHA 独立复验
    ...
    // 头部 v1 定位（不硬编码版本数字）
    assert!(
        s.contains("Higher v1 Windows NSIS Release Build"),
        "V11: 头部为 v1 定位"
    );
}
