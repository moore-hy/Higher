...
fn test_v018_migration_columns_defaults_and_backfill() {
    ...
    assert_eq!(
        (em.is_none(), tk.as_str(), pri.as_str()),
        (true, "structured", "normal"),
        "旧 Task 默认 §120"
    );
    let ak: String = conn
        .query_row(
            "SELECT activity_kind FROM study_sessions LIMIT 1",
            [],
            |r| r.get(0),
        )
...
fn test_write_intent_detection_and_guard_rules() {
    // §8 关键词检测
    assert!(app_lib::ai::prompts::detect_write_intent(
        "帮我今天创建一个任务，读10个英语单词"
    ));
    assert!(app_lib::ai::prompts::detect_write_intent(
        "帮我规划2027考研"
...
fn test_task_groups_and_activity_mapping() {
    ...
    let acc = trepo
        ...
        .unwrap();
    assert_eq!(
        (core.priority.as_str(), core.task_kind.as_str()),
        ("core", "structured")
    );
    assert_eq!(
        (normal.priority.as_str(), normal.task_kind.as_str()),
        ("normal", "structured")
    );
    ...
    assert_eq!(
        srepo.start_quick(p, None).unwrap().activity_kind,
        "unplanned"
    );
    // estimated 边界（§16）
    assert!(trepo
...
fn test_dual_tree_same_session_reference() {
    ...
    let s = StudySessionRepository::new(&conn)
        .start_for_task(p, t.id)
        .unwrap();
    assert_eq!(
        (s.task_id, s.goal_id, s.learning_item_id),
        (Some(t.id), Some(d.id), Some(kb_id))
    );
    // §131：Goal 查询（day 直查 + month/annual/final descendant）
    for gid in [d.id, m.id, y.id, f.id] {
        let found = StudySessionRepository::new(&conn)
            .list_by_goal(p, gid, 10)
            .unwrap();
        assert!(
            found.iter().any(|x| x.id == s.id),
            "goal#{} 应含 session#{}",
            gid,
            s.id
        );
    ...
    let by_item: Vec<i64> = {
        let mut stmt = conn
            ...
            .unwrap();
        stmt.query_map(rusqlite::params![p, kb_id], |r| r.get(0))
            .unwrap()
            .filter_map(|x| x.ok())
            .collect()
    };
    ...
    StudySessionRepository::new(&conn)
        .update_note(s.id, "极限是函数值的趋势（已修改）")
        .unwrap();
    let cnt: i64 = conn
        .query_row(
    ...
    let note: String = conn
        .query_row(
            "SELECT note FROM study_sessions WHERE id=?1",
    ...
    conn.execute(
        "UPDATE tasks SET goal_id=NULL WHERE id=?1",
        rusqlite::params![t.id],
    )
    .unwrap();
    let s_after = StudySessionRepository::new(&conn)
        .get(s.id)
        .unwrap()
        .unwrap();
    assert_eq!(
...
fn test_unassigned_and_organize() {
    ...
    let in_k: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM study_sessions WHERE id=?1 AND learning_item_id=?2",
            rusqlite::params![quick.id, k_id],
            |r| r.get(0),
        )
        .unwrap();
...
fn test_daily_report_metrics_full_formulas() {
    ...
    assert!(
        rep.time_execution_rate.is_none(),
        "未估时 → 时间执行度不可计算"
    );
...
fn test_daily_report_missing_data_handling() {
    ...
    TaskRepository::new(&conn)
        ...
        .unwrap();
    let rep2 = DailyReportRepository::new(&conn)
        .get(p, "2026-08-16")
        .unwrap();
    ...
    assert!(
        (rep2.overall_efficiency.unwrap() - 0.0).abs() < 0.01,
        "0.4*0+0.3*0 重归一后=0"
    );
    let _ = rep2;
    ...
    let m = grepo
        ...
        .unwrap();
    let d = grepo
        .create_tree_node(p, "day", Some(m.id), "8/17", None, Some("2026-08-17"))
        .unwrap();
    let trepo = TaskRepository::new(&conn);
    let a = trepo
        .create_v2(
    ...
    let rep3 = DailyReportRepository::new(&conn)
        .get(p, "2026-08-17")
        .unwrap();
    assert!(
        (rep3.day_goal_progress.unwrap() - 50.0).abs() < 0.01,
        "缺估时改用数量 1/2"
    );
}
...
fn test_changeset_refs_dual_tree_apply_and_invalid_rollback() {
    ...
    repo.apply(cs, p, false).unwrap();
    // §147：真实 FK 正确
    let (g2_parent,): (i64,) = conn
        .query_row(
            "SELECT parent_goal_id FROM goals WHERE name='8月'",
            [],
            |r| Ok((r.get(0)?,)),
        )
        .unwrap();
    let g1_id: i64 = conn
        .query_row("SELECT id FROM goals WHERE name='2026下半年'", [], |r| {
            r.get(0)
        })
        .unwrap();
    assert_eq!(g2_parent, g1_id);
    let (k2_parent,): (Option<i64>,) = conn
        .query_row(
            "SELECT parent_id FROM learning_items WHERE name='极限'",
            [],
            |r| Ok((r.get(0)?,)),
        )
        .unwrap();
    let k1_id: i64 = conn
        .query_row(
            "SELECT id FROM learning_items WHERE name='高等数学'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let (t_goal, t_item, t_kind, t_est): (i64, i64, String, i64) = conn
        ...
        .unwrap();
    let g2_id: i64 = conn
        .query_row("SELECT id FROM goals WHERE name='8月'", [], |r| r.get(0))
        .unwrap();
    let k2_id: i64 = conn
        .query_row(
    ...
    let ghost: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM goals WHERE name='幽灵月'",
            [],
            |r| r.get(0),
        )
        .unwrap();
