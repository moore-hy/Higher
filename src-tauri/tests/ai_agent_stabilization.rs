...
fn setup(name: &str) -> (DbState, VaultState) {
    ...
    let vault_dir =
        std::env::temp_dir().join(format!("higher_dev0066s_{}_{}", name, std::process::id()));
    (
...
fn seed_tree(conn: &Connection, p: i64) -> (i64, i64, i64) {
    let out = run_pack(
        conn,
        &VaultState::new(std::env::temp_dir().join("unused")),
        p,
        0,
        "基座",
        &full_pack_actions(),
    ...
    let m: i64 = conn
        .query_row(
            "SELECT id FROM goals WHERE profile_id=?1 AND goal_level='month'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
...
fn s01_move_containment_cross_profile_and_period() {
    ...
    let p2 = StudyProfileRepository::new(&conn)
        .create("PS2", None, None, None, None, None)
        .unwrap()
        .id;
    conn.execute(
    ...
    assert!(
        st == "invalid_action" || st == "apply_failed",
        "跨档案必须拒绝：{out}"
    );
    let dp: Option<i64> = conn
        .query_row(
            "SELECT parent_goal_id FROM goals WHERE profile_id=?1 AND name='数学复习日'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
    ...
    assert_eq!(
        mv["status"], "apply_failed",
        "2026-09 月移到 2027 年必须被 containment 拒绝：{mv}"
    );
    let mp: Option<i64> = conn
        .query_row(
            "SELECT parent_goal_id FROM goals WHERE profile_id=?1 AND name='2026年9月启动月'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
    let y2026: i64 = conn
        .query_row(
            "SELECT id FROM goals WHERE profile_id=?1 AND name='2026 打基础年'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
...
fn s02_goal_update_undo_restores_name_and_brief() {
    ...
    let n1: String = conn
        .query_row(
            "SELECT name FROM goals WHERE profile_id=?1 AND goal_level='year'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
    ...
    assert_eq!(
        brief["outcome"], "2028 年考取华中科技大学计算机相关专业硕士研究生",
        "undo 后 brief 还原为原值"
    );
}
...
fn s03_goal_target_replace_undo_restores_old_active() {
    ...
    assert!(
        active_title.contains("北京大学"),
        "v2 active：{active_title}"
    );
    ...
    assert!(
        restored.contains("华中科技大学"),
        "undo 后 v1 恢复 active：{restored}"
    );
...
fn s04_full_pack_undo_restores_everything() {
    ...
    let (fin_name, fin_brief): (String, Option<String>) = conn
        .query_row(
...
fn s05_blueprint_activation_no_implicit_task_projection() {
    ...
    let st: String = conn
        .query_row(
            "SELECT status FROM planning_blueprints WHERE profile_id=?1",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
...
fn s08_update_goal_day_kind_only_no_empty_update_op() {
    ...
    let dk: String = conn
        .query_row(
            "SELECT day_kind FROM goals WHERE profile_id=?1 AND name='数学复习日'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
...
fn s09_verify_checks_actual_content() {
    ...
    let gt: (String, String) = conn.query_row(
        ...
        params![p], |r| Ok((r.get(0)?, r.get(1)?))).unwrap();
    assert!(
        gt.0.contains("华中科技大学") && gt.1 == "2027-12-25",
        "GT 实际内容：{gt:?}"
    );
    let ph: (String, String) = conn
        .query_row(
    ...
    let p1: i64 = conn
        .query_row(
            "SELECT id FROM planning_phases WHERE phase_key='P1'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn s10_provider_error_closes_run_as_failed() {
    ...
    let (p, c, m) = {
        ...
        let msg = ConversationRepository::new(&conn)
            .add_message(f.1, f.0, "user", "帮我规划", None)
            .unwrap();
        (f.0, f.1, msg.id)
    };
    ...
    let conn = state.0.lock().unwrap();
    let (status, error_flag): (String, String) = conn
        .query_row(
            "SELECT status, COALESCE(error,'') FROM ai_runs WHERE id=?1",
            params![RUN_ID],
            |r| Ok((r.get(0)?, r.get(1)?)),
        )
        .unwrap();
    assert_eq!(
        status, "failed",
        "ai_runs.status 必须 failed（不得留 running）"
    );
    assert_eq!(
        error_flag, "agent_runtime_error",
        "error 标记：{error_flag}"
    );
    let wf_state: String = conn
        .query_row(
            "SELECT workflow_state FROM ai_runs WHERE id=?1",
            params![RUN_ID],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(
        wf_state, "failed",
        "workflow_state 必须 failed（不得留 understanding）"
    );
}
