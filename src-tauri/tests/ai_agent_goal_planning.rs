...
fn setup(name: &str) -> (DbState, VaultState) {
    ...
    let vault_dir =
        std::env::temp_dir().join(format!("higher_dev0066d_{}_{}", name, std::process::id()));
    (
...
fn d01_set_reach_and_safety_active() {
    ...
    assert!(
        inst.contains("institution_name") && inst.contains("program_name"),
        "postgraduate data_json：{inst}"
    );
}
...
fn d02_repeat_goal_target_no_duplicate() {
    ...
    assert_eq!(
        ...
        2,
        "仍然只有 REACH+SAFETY 两条：{}",
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goal_targets WHERE profile_id=?1",
            &[&p]
        )
    );
    let cs_after: i64 = conn
        .query_row(
...
fn d03_final_goal_brief_and_insufficient() {
    ...
    assert_eq!(
        bad["status"], "insufficient_information",
        "缺 outcome 必须拒绝：{bad}"
    );
...
fn d04_goal_tree_parent_relations() {
    ...
    assert_eq!(
        bad["status"], "apply_failed",
        "day 挂 year 必须整包回滚：{bad}"
    );
    let n_day: i64 = conn
        .query_row(
...
fn d06_blueprint_phases_milestones_exist() {
    ...
    let phases: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM planning_phases WHERE blueprint_id=?1",
            params![bp_id],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let p1_id: i64 = conn
        .query_row(
            "SELECT id FROM planning_phases WHERE blueprint_id=?1 AND phase_key='P1'",
            params![bp_id],
            |r| r.get(0),
        )
        .unwrap();
    let m1_phase: Option<i64> = conn
        .query_row(
...
fn d07_full_pack_one_changeset_all_success() {
    ...
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goals WHERE profile_id=?1 AND goal_level='final'",
            &[&p]
        )["n"],
        1
    );
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goals WHERE profile_id=?1 AND goal_level='year'",
            &[&p]
        )["n"],
        2
    );
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM planning_blueprints WHERE profile_id=?1 AND status='active'",
            &[&p]
        )["n"],
        1
    );
...
fn d08_invalid_action_rolls_back_whole_pack() {
    ...
    assert_eq!(
        overlap["status"], "apply_failed",
        "引擎年重叠校验触发整包回滚：{overlap}"
    );
    let n_year: i64 = conn
        .query_row(
...
fn d09_repeat_full_pack_idempotent() {
    ...
    assert_eq!(
        second["message"].as_str().unwrap_or(""),
        "全部动作均为已有状态（幂等 no-op，未产生重复数据）",
        "{second}"
    );
    // 无任何重复结构
    assert_eq!(
    ...
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goals WHERE profile_id=?1 AND goal_level='month'",
            &[&p]
        )["n"],
        1,
        "无重复月份"
...
fn d10_modify_existing_updates_in_place() {
    ...
    let name: String = conn
        .query_row(
            "SELECT name FROM goals WHERE profile_id=?1 AND goal_level='month'",
            params![p],
            |r| r.get(0),
        )
        .unwrap();
    ...
    assert_eq!(
        bad_mv["status"], "apply_failed",
        "9月day 移到 10月month 必须被 containment 拒绝：{bad_mv}"
    );
    ...
    let (n_active, n_total): (i64, i64) = conn
        .query_row(
...
fn d11_read_back_verify_all_domains() {
    ...
    assert_eq!(
        out["verification"]["checked_ops"], 17,
        "全部 17 个 op 逐一回读：{out}"
    );
    // 独立复核（不信任工具自报）：五域数据与 DB 一致
    assert!(one(
        &conn,
        "SELECT goal_brief_json AS b FROM goals WHERE profile_id=?1 AND goal_level='final'",
        &[&p]
    )["b"]
        .as_str()
        .unwrap_or("")
    ...
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goal_targets WHERE profile_id=?1 AND status='active'",
            &[&p]
        )["n"],
        2
    );
    assert_eq!(
        one(
            &conn,
            "SELECT COUNT(*) AS n FROM goals WHERE profile_id=?1",
            &[&p]
        )["n"],
        5
    );
    assert_eq!(
        one(&conn, "SELECT COUNT(*) AS n FROM planning_phases", &[])["n"],
        4
    );
    assert_eq!(
        one(&conn, "SELECT COUNT(*) AS n FROM planning_milestones", &[])["n"],
        2
    );
}
...
fn d12_single_entry_no_separate_goal_or_planning_ai() {
    ...
    app_lib::migrations::run_migrations(&conn).unwrap();
    let p = StudyProfileRepository::new(&conn)
        .create("P12", None, None, None, None, None)
        .unwrap()
        .id;
    let c = ConversationRepository::new(&conn)
        .create(p, "assistant", "D12")
        .unwrap()
        .id;
    let vault = VaultState::new(
        std::env::temp_dir().join(format!("higher_dev0066d_d12_{}", std::process::id())),
    );
    // 五类能力在同一入口下都能走通（简化版）
    let out = run_pack(&conn, &vault, p, c, "统一入口验证", &full_pack_actions());
    assert_eq!(
        out["status"], "applied",
        "五类写能力全部经 execute_higher_actions：{out}"
    );
    assert_eq!(out["verified"], true);
}
