...
use app_lib::repository::{
    ...
    note,
    recurring_rule::{
        materialize_recurring_tasks, rule_matches_date, weekday_of, RecurringRuleRepository,
    },
...
fn test_v010_schema_and_old_data_preserved() {
    let conn = setup();
    // v010 表 + 列存在；版本记录 32 条
    // （DEV-0076 §四：v027 / DEV-SYNC-001：v028 / DAILY EXPERIENCE V1 §PHASE 4：v032）
    let count: i64 = conn
...
fn test_note_v2_save_read_and_media_parse() {
    ...
    let blocks = note::serialize_blocks(&[
        ...
        note::NoteBlock::Drawing {
            a: 12,
            n: "画图.png".into(),
        },
        note::NoteBlock::Video {
            a: 13,
            n: "讲解.mp4".into(),
        },
    ]);
    StudySessionRepository::new(&conn)
        .update_note(sess.id, &blocks)
        .unwrap();

    let loaded = StudySessionRepository::new(&conn)
        .get(sess.id)
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        note::plain_text(loaded.note.as_deref()),
        "第一段：等价无穷小\n[图片]\n第二段：洛必达\n[画图]\n[视频]"
    );
...
fn test_today_task_crud_flow() {
    ...
    for i in 1..=5 {
        let t = repo
            .create(s.item, &format!("任务{}", i), Some("2026-08-15"))
            .unwrap();
        ids.push(t.id);
    }
    ...
    // 原 08-15 只剩 4
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, "2026-08-15", "2026-08-15")
            .unwrap()
            .len(),
        4
    );
...
fn test_today_profile_isolation_and_sort() {
    ...
    let repo = TaskRepository::new(&conn);
    let today = today_str();
    repo.create_with_plan_legacy(s.item, "A任务", Some(&today), None)
        .unwrap();
    repo.create_with_plan_legacy(other_item.id, "B任务", Some(&today), None)
        .unwrap();
    repo.create_with_plan_legacy(s.item, "A无时间", Some(&today), None)
        .unwrap();
    repo.complete(
        repo.create_with_plan_legacy(s.item, "A完成", Some(&today), None)
            .unwrap()
            .id,
    )
    .unwrap();
...
fn test_daily_rule_materialize_and_idempotency() {
    ...
    assert_eq!(
        materialize_recurring_tasks(&conn, s.profile, future).unwrap(),
        1
    );
}
...
fn test_weekly_rule_weekday_selection() {
    ...
    // 周日不生成、周一生成
    assert_eq!(
        materialize_recurring_tasks(&conn, s.profile, "2026-08-16").unwrap(),
        0
    );
    assert_eq!(
        materialize_recurring_tasks(&conn, s.profile, "2026-08-17").unwrap(),
        1
    );
}
...
fn test_rule_start_end_disabled_and_edit() {
    ...
    assert_eq!(
        materialize_recurring_tasks(&conn, s.profile, "2026-08-31").unwrap(),
        0
    );
...
fn test_delete_rule_keeps_history_and_profile_isolation() {
    ...
    // materialize 也按档案隔离
    assert_eq!(
        materialize_recurring_tasks(&conn, other.id, &today).unwrap(),
        0
    );
}
...
fn test_review_sessions_and_summary_data() {
    ...
    sess.update_note(t1.id, "今天重新理解了极限的定义，ε-δ 语言终于通了")
        .unwrap();
...
fn test_knowledge_crud_move_guards() {
    ...
    assert!(
        repo.move_item(s.item, Some(child.id)).is_err(),
        "不能移到自己的后代下"
    );
    let other = StudyProfileRepository::new(&conn)
        .create("B", None, None, None, None, None)
        .unwrap();
    let other_goal = GoalRepository::new(&conn)
        .create(other.id, "另一目标", None)
        .unwrap();
    let other_item = repo.create_root(other_goal.id, "别的", None).unwrap();
    assert!(
        repo.move_item(child.id, Some(other_item.id)).is_err(),
        "不能跨 Goal"
    );
...
fn test_progress_zero_denominator_and_stage_clamp() {
    ...
    stages
        .create(s.goal, "阶段", None, Some("2026-01-01"), Some("2026-01-31"))
        .unwrap();
    // 激活该阶段
    let list = stages.list_by_goal(s.goal).unwrap();
    let sid = list[0].id;
    conn.execute(
        "UPDATE study_stages SET status='active' WHERE id=?1",
        rusqlite::params![sid],
    )
    .unwrap();
...
fn test_progress_30day_trend_and_profile_isolation() {
    ...
    assert!(other_trend
        .iter()
        .all(|t| t.completed_tasks == 0 && t.session_count == 0));
}
...
fn test_cleanup_preview_clear_today_execute() {
    ...
    // 预览（只读；不执行）
    let p = cleanup
        .preview(s.profile, CleanupScope::ClearToday, &today)
        .unwrap();
    assert_eq!(p.tasks, 1);
    ...
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, &yesterday, &yesterday)
            .unwrap()
            .len(),
        1
    );
...
fn test_cleanup_keep_today_and_month_year() {
    ...
    // keep_today：保留今天，删其他
    cleanup
        .execute_collecting(s.profile, CleanupScope::KeepToday, &today)
        .unwrap();
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, &today, &today)
            .unwrap()
    ...
    cleanup
        .execute_collecting(s.profile, CleanupScope::ClearMonth, &today)
        .unwrap();
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, &yesterday, &yesterday)
            .unwrap()
    ...
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, &last_month, &last_month)
            .unwrap()
            .len(),
        1
    );
    ...
    cleanup
        .execute_collecting(s.profile, CleanupScope::KeepYear, &today)
        .unwrap();
    assert_eq!(
        repo.list_by_range_by_profile(s.profile, &last_year, &last_year)
            .unwrap()
            .len(),
        0
    );
...
fn test_cleanup_full_reset_keeps_profile_shell() {
    ...
    let cleanup = CleanupRepository::new(&conn);
    let p = cleanup
        .preview(s.profile, CleanupScope::FullReset, &today)
        .unwrap();
    assert_eq!(p.goals, 1);
...
fn test_cleanup_profile_isolation() {
    ...
    let today = today_str();
    TaskRepository::new(&conn)
        .create(s.item, "A任务", Some(&today))
        .unwrap();
    TaskRepository::new(&conn)
        .create(other_item.id, "B任务", Some(&today))
        .unwrap();
    ...
    let b_tasks = TaskRepository::new(&conn)
        .list_all_by_profile(other.id)
        .unwrap();
    assert_eq!(b_tasks.len(), 1, "清理 A 档案不影响 B");
    let a_tasks = TaskRepository::new(&conn)
        .list_all_by_profile(s.profile)
        .unwrap();
