...
fn ui_tc004_006_bottomnav() {
    ...
    assert!(
        layout.contains("IconCalendar") && layout.contains("IconPerson"),
        "UI-TC006: SVG 图标"
    );
    for emoji in ["📅", "🧭", "🗂", "✦", "⚙"] {
    ...
    assert!(
        icons.contains("stroke: \"currentColor\"") && icons.contains("strokeWidth: 1.8"),
...
fn ui_tc005_ai_fullscreen() {
    let layout = web("mobile/MobileLayout.tsx");
    assert!(
        layout.contains("ai-slot") && layout.contains("<AiPanel presentation=\"mobile\" />"),
        "UI-TC005: AiPanel 常驻 main 内 ai-slot"
    );
    assert!(
        !layout.contains("mobile-ai-host"),
        "UI-TC005: 无 fixed Overlay host（F1-A）"
    );
    let css = css();
    let mcss = web("mobile/mobile.css"); // F1 新规则唯一所在（Design System）
    assert!(
        mcss.contains(".ai-slot--visible") && mcss.contains(".mobile-main--ai"),
    ...
    assert!(
        panel.contains("aipanel__mobile-subtitle") && panel.contains("pageContext.pageLabel"),
        "UI-TC005: 保留来源上下文 subtitle（F1-C）"
    );
    // F1-C：进入 /ai 不重置 pageContext
    assert!(
        layout.contains("保留上一个业务页注入的上下文") || layout.contains("p.startsWith(\"/ai\")"),
        "UI-TC005: /ai 不重置上下文"
    );
    // F1-D/E：safe-area 根统一 + BottomNav max(safe-bottom,8px)
    assert!(
        mcss.contains(".mobile-layout {\n  padding-top: env(safe-area-inset-top)"),
        "UI-TC005: safe-top 根统一（D）"
    );
    assert!(
        mcss.contains("max(env(safe-area-inset-bottom), 8px)"),
...
fn ui_tc007_008_safearea_touch() {
    ...
    assert!(
        css.contains("100dvh") || mcss.contains("100dvh"),
        "UI-TC007: 100dvh 视口"
    );
    assert!(
        mcss.contains("min-height: 44px") || css.contains("min-height: 44px"),
        "UI-TC008: ≥44px 触控目标"
    );
}
...
fn ui_tc009_knowledge_drawer() {
    ...
    assert!(
        k.contains("treeDrawerOpen") && k.contains("knowledge__tree--drawer"),
        "UI-TC009: Knowledge 树 Drawer 机制"
    );
    assert!(
        k.contains("knowledge__tree-toggle"),
        "UI-TC009: 目录切换按钮"
    );
    let css = css();
    assert!(
        css.contains(".knowledge__tree--drawer") && css.contains("position: fixed"),
...
fn ui_tc010_settings_mobile_navigation() {
    ...
    assert!(
        ms.contains("msettings__list") && ms.contains("msettings__item"),
        "UI-TC010: section 列表"
    );
    assert!(
        ms.contains("<Settings initialTab={section} presentation=\"mobile-section\" />"),
        "UI-TC010: 进入对应 section（复用 Settings mobile-section，零业务复制）"
    );
    let app = web("App.tsx");
    assert!(
        app.contains("IS_ANDROID ? <MobileSettings /> : <Settings />"),
        "UI-TC010: /settings Android 路由切「我的」"
    );
    let settings = web("pages/Settings.tsx");
    assert!(
        settings.contains("initialTab") && settings.contains("initialTab ?? \"profile\""),
