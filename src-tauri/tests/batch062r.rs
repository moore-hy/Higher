...
use app_lib::ai::provider::{
    control_known_false, resolve_active_ai_profiles, with_forced_json, AdapterKind, AiCapabilities,
    AiRuntimeConfig, JsonStrategy, ThinkingMode,
};
            ...
            for resp in script {
                ...
                let (status, payload, ctype) = match resp {
                    ...
                    FakeResponse::Status(code, b) => (
                        format!("{code} Internal Server Error"),
                        b,
                        "application/json",
                    ),
                };
...
fn r01_pure_json_pass() {
    assert!(
        structured_output_valid("{\"route\":\"fast_chat\",\"skills\":[]}"),
        "R01"
    );
}
...
fn r03_wrong_shape_fail() {
    assert!(
        !structured_output_valid("{\"abc\":123}"),
        "R03: 错 shape 必须 FAIL"
    );
}
...
fn r04_prose_around_json_fail() {
    ...
    assert!(
        !structured_output_valid("{\"route\":\"fast_chat\"}{\"route\":\"higher_read\"}"),
        "R04: 多个 JSON FAIL"
    );
}
...
fn r05_force_native_independent_of_history() {
    ...
    assert!(
        !cfg.use_native_json(true),
        "R05: 历史 prompt_only → 正式请求禁发"
    );
    ...
    assert!(
        !forced_po.use_native_json(true),
        "R05: ForcePromptOnly 强制禁发"
    );
...
fn r06_native_valid_no_promptonly() {
    ...
    assert_eq!(
        structured_reqs, 1,
        "R06: PromptOnly 请求 0 次（Native 直接通过）"
    );
}
...
fn r07_native_200_invalid_falls_to_promptonly() {
    ...
    assert!(
        structured[0].contains("\"response_format\""),
        "R07: 第一次是 Native"
    );
    assert!(
        !structured[1].contains("\"response_format\""),
        "R07: 第二次禁发 response_format"
    );
}
...
fn r11_structured_temperature_zero() {
    ...
    for (i, b) in structured.iter().enumerate() {
        assert!(
            b.contains("\"temperature\":0.0"),
            "R11: 第{}个 structured 请求 temp=0.0",
            i + 1
        );
    }
}
...
fn r13_probe_zero_formal_writes() {
    ...
    assert_eq!(
        saved.compatibility_status, out.status,
        "R13: probe 结果正常持久化"
    );
}
...
fn r15_basic_chat_empty_false_and_stops() {
    ...
    assert_eq!(
        ...
        "R15: basic=false 仍 incompatible"
    );
    assert_eq!(
        out.capabilities.structured_json,
        Some(true),
    ...
    assert!(
        out.failures.contains(&"empty_content"),
        "R15: 空内容失败类别"
    );
}
...
fn r20_temp0_empty_false() {
    ...
    let out = block_on(run_probe(&fake_cfg(&fp.base_url())));
    assert_eq!(
        out.capabilities.temperature_zero,
        Some(false),
        "R20: content.is_some 不足以判成功"
    );
}
...
fn r23_basic_false_control_reject() {
    ...
    assert!(
        ...
        "R23: basic=false → Provider 调用前 deterministic reject"
    );
}
...
fn r27_untested_legacy_active_not_blocked() {
    ...
    assert!(
        !control_known_false(&caps),
        "R27: None ≠ Known False（不因本轮修复硬阻断）"
    );
}
...
fn r29_primary_disabled_error_no_fallback() {
    let conn = setup();
    conn.execute("UPDATE ai_provider_profiles SET enabled=0 WHERE id=1", [])
        .unwrap();
    let err = resolve_active_ai_profiles(&conn).unwrap_err();
    assert!(
        err.contains("停用"),
        "R29: 不 fallback 到其他 enabled（{err}）"
    );
}
...
fn r30_control_missing_error_no_follow_primary() {
    ...
    assert!(
        err.contains("动作理解"),
        "R30: 显式 Control 失效禁止偷偷 Follow Primary（{err}）"
    );
}
...
fn r32_control_none_follows_primary() {
    ...
    assert!(
        ...
        "R32: control id 空 = 唯一合法 Follow Primary"
    );
    assert_eq!(
        resolved.control.profile_id, resolved.primary.profile_id,
        "R32"
    );
}
...
fn r33_legacy_save_settings_no_first_enabled() {
    ...
    assert!(
        err.contains("主要 AI"),
        "R33: 引用失效明确报错，禁止 first-enabled（{err}）"
    );
}
...
fn r34_atomic_switch_no_partial_state() {
    ...
    assert!(
        repo.set_active_profiles_atomic(p2, Some(weak)).is_err(),
        "R34: Control invalid → ERROR"
    );
    assert_eq!(
        repo.active_primary_id(),
        before_primary,
        "R34: Primary 原值不变"
    );
    assert_eq!(
        repo.active_control_id(),
        before_control,
        "R34: Control 原值不变"
    );
}
...
fn r35_atomic_switch_both_updated() {
    ...
    let p2 = full_profile(&conn, "P2");
    assert!(
        repo.set_active_profiles_atomic(p2, Some(p2)).is_ok(),
        "R35: 两者合法 → 同事务成功"
    );
...
fn r37_untested_other_id_rejected() {
    ...
    let fresh = repo
        ...
        .unwrap();
    assert!(
        repo.set_active_profiles_atomic(fresh, None).is_err(),
        "R37: 切到新 untested 拒绝"
    );
...
fn r38_disable_active_primary_rejected() {
    ...
    assert!(
        repo.get(pid).unwrap().unwrap().enabled,
        "R38: enabled 保持 true"
    );
}
...
fn r39_disable_explicit_control_rejected() {
    ...
    assert!(
        repo.get(p2).unwrap().unwrap().enabled,
        "R39: enabled 保持 true"
    );
}
...
fn r42_probe_report_no_key_no_authorization() {
    ...
    let out = block_on(run_probe(&cfg));
    assert!(
        !out.message.contains(&cfg.api_key),
        "R42: message 无 API Key"
    );
    assert!(
        !out.message.contains("Authorization"),
        "R42: 无 Authorization"
    );
    let conn = setup();
    let repo = AiProviderProfileRepository::new(&conn);
    ...
    let saved = repo.get(id).unwrap().unwrap();
    assert!(
        !saved.last_test_message.contains(&cfg.api_key),
        "R42: last_test_message 无 Key"
    );
}
...
fn r43_trace_no_key_no_full_prompt() {
    ...
    assert!(
        comp.contains("chars().take(120)"),
        "R43: repair 输入对 raw 输出截断"
    );
}
...
fn r44_no_raw_response_persisted() {
    ...
    let out = block_on(run_probe(&cfg));
    assert!(
        !out.message.contains(marker),
        "R44: message 不含 raw response"
    );
    let conn = setup();
    let repo = AiProviderProfileRepository::new(&conn);
    let id = repo
    ...
    let saved = repo.get(id).unwrap().unwrap();
    assert!(
        !saved.last_test_message.contains(marker),
        "R44: DB 无 raw response"
    );
}
