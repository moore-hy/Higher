...
fn t1_unique_candidate_grounds_without_title_match() {
    ...
    match out {
        GroundingOutcome::Resolved(id) => {
            assert!(id > 0, "T1: 唯一候选直接 Ground（title 不必一致）")
        }
...
fn t3_multiple_candidates_need_selection() {
    ...
    match ground_single("英语", cands, None) {
        GroundingOutcome::Ambiguous(c) => {
            assert_eq!(c.len(), 2, "T3: 无 selection → Ambiguous 澄清")
        }
...
fn t4_invalid_model_candidate_id() {
    ...
    assert_eq!(
        ...
        "T4: 幻想 candidate_id → Invalid"
    );
    match ground_single("英语", cands, Some(&fake)) {
        GroundingOutcome::Ambiguous(c) => {
            assert_eq!(c.len(), 2, "T4: Invalid → 安全澄清，不得 Ground")
        }
...
fn t5_model_ambiguous_no_changeset() {
    ...
    assert!(
        matches!(sel, SelectionOutcome::Ambiguous(_)),
        "T5: 模型 ambiguous 解析成功"
    );
    ...
    match out {
        ActionOutcome::Clarification { message: msg, .. } => {
            ...
            assert!(
                msg.contains("背10个英语单词") && msg.contains("复习英语单词"),
                "T5: 列出候选"
            );
        }
    ...
    assert_eq!(
        ...
        0,
        "T5: 澄清 0 ChangeSet（AI-GND-009/§20.1）"
    );
}
...
fn t7_recurring_semantic_selection() {
    ...
    let target_cid = cands
        .iter()
        .find(|c| c.title == "背10个英语单词")
        .unwrap()
        .candidate_id
        .clone();
...
fn t9_recent_plural() {
    ...
    match resolve_recent(&conn, p, CONV_RECENT_T9, &h).unwrap() {
        GroundingOutcome::ResolvedMany(ids) => {
            assert_eq!(ids.len(), 2, "T9: 刚才创建的两个 → ResolvedMany 2")
        }
...
fn t10_recent_empty_no_hallucination() {
    ...
    let out = plan_action(
        ...
        p,
        &env(),
        &default_plan("把刚才那个改成20分钟"),
        &act,
    )
    .unwrap();
    assert!(
        matches!(out, ActionOutcome::NotFound(_)),
        "T10: plan → NotFound"
    );
...
fn t11_t13_target_scope() {
    ...
    let del = SemanticAction::DeleteTask {
    ...
    let cur = SemanticAction::SetTaskStatus {
        target: ch,
...
fn t14_real_fail_update_today_task() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("run-t14"), "t", "t", &ops)
        .unwrap();
...
fn t16_not_found_never_creates_changeset() {
    ...
    if let ActionOutcome::ProposalReady { .. } = out {
        unreachable!()
    }
    assert_eq!(
        count(&conn, "ai_change_sets"),
        0,
        "T16: ChangeSetRepository::create NOT CALLED"
    );
}
...
fn t18_no_internal_error_text_leaks() {
    ...
    assert!(
        !lib.replace("提案生成失败：{e}", "")
            .contains(&format!("format!(\"{banned}")),
        "T18: lib.rs 不得直接展示内部错误"
    );
...
fn mk_today_three(conn: &Connection, p: i64) {
    ...
    repo.create_for_profile(p, None, "T1", Some("2026-08-21"), None, None, None)
        .unwrap();
    repo.create_for_profile(p, None, "T2", Some("2026-08-21"), None, None, None)
        .unwrap();
    let t3 = repo
        .create_for_profile(p, None, "T3", Some("2026-08-21"), None, None, None)
        .unwrap();
    conn.execute(
...
fn t19_t20_bulk_reschedule() {
    ...
    let ops = match out {
        ActionOutcome::ProposalReady {
            ops,
            scope,
            selection_provider_called,
            ..
        } => {
            assert_eq!(scope, TargetScope::MatchedSet);
            assert!(
                !selection_provider_called,
                "T19: bulk 结构动作 0 selection call"
            );
            ops
        }
    ...
    validate_ops(&e, &act, &ops).unwrap();
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("run-t19"), "b", "b", &ops)
        .unwrap();
    ...
    let moved: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tasks WHERE planned_date='2026-08-22' AND status!='completed'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    let stayed: Option<String> = conn
        .query_row(
            "SELECT planned_date FROM tasks WHERE status='completed'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn t21_bulk_over_limit_scope_too_broad() {
    ...
    match out {
        ActionOutcome::Clarification { message: msg, .. } => {
            assert!(
                msg.contains("上限") || msg.contains("缩小"),
                "T21: ScopeTooBroad 文案：{msg}"
            );
        }
...
fn t22_delete_occurrence_rule_unchanged() {
    ...
    let act = SemanticAction::DeleteTask {
        target: hint("task", "背单词", true),
    };
    let out = plan_action(
        &conn,
        p,
        &e,
        &default_plan("删除今天这条背单词任务，但每日规则继续"),
        &act,
    )
    .unwrap();
    let ops = match out {
    ...
    validate_ops(&e, &act, &ops).unwrap();
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("r22"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    let enabled: bool = conn
        .query_row(
            "SELECT enabled FROM recurring_task_rules WHERE id=?1",
            params![rule.id],
            |r| r.get(0),
        )
        .unwrap();
...
fn t23_disable_series_keeps_today_and_past() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("r23"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    let enabled: bool = conn
        .query_row(
    ...
    // 未来不再生成
    assert_eq!(
        app_lib::repository::recurring_rule::materialize_recurring_tasks(&conn, p, "2026-08-22")
            .unwrap(),
        0
    );
}
...
fn t24_series_update_syncs_future() {
    ...
    let rule_op = ops
        .iter()
        .find(|o| o.entity_type == "recurring_rule")
        .expect("rule update op");
    ...
    validate_ops(&e, &act, &ops).unwrap();
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("r24"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
...
fn t25_t26_past_and_completed_untouched() {
    ...
    TaskRepository::new(&conn)
        ...
        .unwrap();
    conn.execute(
        "UPDATE tasks SET status='completed' WHERE planned_date='2026-08-23'",
        [],
    )
    .unwrap();
    let act = SemanticAction::UpdateRecurringTask {
    ...
    assert_eq!(
        task_ops.len(),
        1,
        "T25/T26: 只有明天 pending 被同步（昨天/Completed 不动）"
    );
    // apply 后昨天与 completed 保持 20:00
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("r25"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    let y: Option<String> = conn
        .query_row(
            "SELECT planned_time FROM tasks WHERE planned_date='2026-08-20'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    let c: Option<String> = conn
        .query_row(
            "SELECT planned_time FROM tasks WHERE planned_date='2026-08-23'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn t27_disable_cleans_future_pending() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("r27"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    let n: i64 = conn
        .query_row("SELECT COUNT(*) FROM tasks", [], |r| r.get(0))
...
fn t28_t31_provider_budget() {
    ...
    match plan_action(&conn, p, &e, &default_plan("建任务"), &create).unwrap() {
        ActionOutcome::ProposalReady {
            ...
            assert!(
                !selection_provider_called,
                "T28: create grounding/selection = 0"
            );
        }
        other => panic!("{other:?}"),
    }
    ...
    match plan_action(&conn, p, &e, &default_plan("改30分钟"), &upd_unique).unwrap() {
        ActionOutcome::ProposalReady {
            ...
            assert!(
                !selection_provider_called,
                "T29: unique candidate selection = 0"
            );
        }
        other => panic!("{other:?}"),
    }
...
fn t32_model_invented_ids_ignored() {
    ...
    let h: EntityHint =
        serde_json::from_value(json!({"title_hint":"x","task_id":"invented"})).unwrap();
    assert!(serde_json::to_string(&h).unwrap().contains("title_hint"));
    // Candidate guard 同 T4
    let cands = vec![app_lib::ai::grounding::Candidate {
...
fn t34_pre_approval_mutation_zero() {
    ...
    if let ActionOutcome::ProposalReady { ops, .. } = out {
        let cs = ChangeSetRepository::new(&conn)
            .create(p, None, Some("run-t34"), "t", "t", &ops)
            .unwrap();
        // Proposal 创建后、Apply 前：正式数据 0 修改
        assert_eq!(
            count(&conn, "tasks"),
            before_tasks,
            "T34: pre-approval task 不变"
        );
        assert_eq!(
            count(&conn, "recurring_task_rules"),
            before_rules,
            "T34: pre-approval rule 不变"
        );
        let est: Option<i64> = conn
            .query_row("SELECT estimated_minutes FROM tasks LIMIT 1", [], |r| {
                r.get(0)
            })
            .unwrap();
...
fn t35_grounding_prompt_minimal_no_source_scan() {
    ...
    for banned in [
        ...
        assert!(
            ...
            "T35: selection prompt 禁止包含 {banned}"
        );
    }
    assert!(
        prompt.contains("背10个英语单词") && prompt.contains("T-"),
        "T35: 只含候选 DTO"
    );
...
fn recent_record_apply_from_changeset() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, Some("run-recent"), "t", "t", &ops)
        .unwrap();
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    app_lib::ai::grounding::record_apply(&conn, p, CONV_RECENT_APPLY, cs);
    {
        ...
        let ctx = map
            .get(&(p, CONV_RECENT_APPLY))
            .expect("Apply 后 (p,CONV) 条目存在");
        assert_eq!(
            ctx.last_created_task_ids.len(),
            1,
            "Apply 后 create 的真实 id 进入 Recent"
        );
    }
    // H7 语义：之后"刚才那个"能指到它
    let mut h = EntityHint::default();
