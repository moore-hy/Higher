...
use app_lib::ai::planner::{
    ...
    set_workflow_payload, validate_plan_draft, PlannerQuestion, PlanningContinuation,
    PlanningWorkflowPayload, TargetProposalDraft, ToolRoundOutcome, WORKFLOW_STATE_CANCELLED,
...
fn test_t2_duplicate_content_excluded_by_id_only() {
    ...
    let user_nihao: Vec<&app_lib::ai::client::ChatMessage> = msgs
        .iter()
        .filter(|m| m.role == "user" && m.content == "你好")
        .collect();
    assert_eq!(
        user_nihao.len(),
        2,
        "历史旧「你好」必须保留 + 当前「你好」在最后：实际 {user_nihao:?}"
    );
    assert_eq!(msgs.last().unwrap().content, "你好");
    assert!(msgs
        .iter()
        .any(|m| m.role == "user" && m.content == "你好" && msgs.len() > 0));
...
fn test_t3_context_is_background_not_user() {
    ...
    assert!(
        msgs.iter()
            ...
            .all(|m| !m.content.contains(ctx_marker)),
        "Context 不得冒充 User Message"
    );
    // Context 在 system 消息中，且带背景声明
    let ctx_sys = msgs
        .iter()
        .find(|m| m.role == "system" && m.content.contains(ctx_marker))
        .unwrap();
...
fn test_t4_generic_context_minimal() {
    ...
    let mem_id = mem_repo
        .create_pending_memory(&MemoryRecord {
            id: 0,
            profile_id: p,
            memory_type: "user_fact".into(),
            category: "chat".into(),
            memory_key: "chat::m".into(),
            memory_value: "用户偏好晚上学习".into(),
            source_kind: "user_message".into(),
            source_ref: String::new(),
            source_excerpt: "我晚上学习".into(),
            importance: 3,
    ...
    // 检测：Generic
    assert_eq!(
        detect_context_purpose(q, &page, false),
    ...
    let all = report
        ...
        .collect::<String>();
    assert!(
        !all.contains("每天可学 3 小时"),
        "Generic 不得注入 PersonalProfile：{all}"
    );
    assert!(!all.contains("当前目标"), "Generic 不得注入当前目标：{all}");
    assert!(
        !all.contains("用户偏好晚上学习"),
        "Generic 不得注入 Memory：{all}"
    );
    assert!(
        report.chips.iter().all(|c| c == "当前上下文"),
        "Generic 只保留 L1：{:?}",
        report.chips
    );
    ...
    assert_eq!(
        detect_context_purpose(q2, &page, false),
        ContextPurpose::Personal
    );
...
fn test_t5_canonical_goaltarget_beats_legacy() {
    ...
    let report = build_context(
        &conn,
        p,
        "我的目标是什么",
        &page,
        "readonly",
        ContextPurpose::HigherData,
    )
    .unwrap();
    let l1 = &report.layers[0].text;
    assert!(
        l1.contains("华中科技大学"),
        "当前目标必须是 active GoalTarget：{l1}"
    );
    assert!(
        !(l1.contains("当前目标：考研2027") || l1.contains("当前目标：考上清华")),
        "禁止 legacy 冒充当前目标：{l1}"
    );
...
fn test_t6_no_goaltarget_legacy_not_promoted() {
    ...
    assert!(
        l1.contains("正式目标未设置"),
        "无 GoalTarget 必须如实说明：{l1}"
    );
    assert!(
        !l1.contains("当前目标：考研2027"),
        "禁止 legacy 晋升为当前目标：{l1}"
    );
    ...
    assert!(
        !v["legacy_candidates"].as_array().unwrap().is_empty(),
        "legacy 仅作候选可见"
    );
}
...
fn test_t7_tool_definition_allowlist_equality() {
    ...
    assert_eq!(
        defined, allow,
        "definition 与 allowlist 必须集合一致（不允许看得见调不得）"
    );
    for t in [
        ...
        assert!(
            TOOL_ALLOWLIST.contains(&t),
            "planning read tool {t} 必须在 allowlist（READ 分类）"
        );
        assert!(
            !app_lib::ai::tools::ASSISTANT_TOOLS.contains(&t),
            "{t} 不是 Assistant-only"
        );
    }
    ...
    let direct_write: Vec<&String> = allow
        ...
        .filter(|t| {
            let t = t.as_str();
            (t.starts_with("create_")
                || t.starts_with("update_")
                || t.starts_with("delete_")
                || t.starts_with("apply_")
                || t.starts_with("write_"))
                && t != "propose_change_set"
        })
        .collect();
    assert!(
        direct_write.is_empty(),
        "Direct Write Tools 必须为 0：{direct_write:?}"
    );
}
...
fn test_t8_workflow_latest_by_created_at_not_uuid() {
    ...
    assert_eq!(
        st, "applied",
        "workflow latest 必须按 created_at（真实时间）取，不是 UUID 字典序"
    );
}
...
fn test_t9_clarification_continue_absorbs_reply() {
    ...
    assert!(
        !reply.contains("目标院校"),
        "新一轮 clarification 不含已回答字段：{reply}"
    );
}
...
fn test_t11_planner_exit() {
    ...
    let mut payload = PlanningWorkflowPayload {
        original_request: "帮我制定计划".into(),
        ..Default::default()
    };
    payload.pending_questions = vec![PlannerQuestion {
        key: "k".into(),
        question: "q".into(),
    }];
    set_workflow_payload(
        &conn,
        "run-cancel",
        p,
        conv,
    ...
    assert!(
        !app_lib::ai::planner::workflow_active(&st),
        "cancelled 必须 inactive（不劫持后续消息）"
    );
    // 正式数据 0 修改（取消路径不产生任何 op/changeset）
    let gt: i64 = conn
        .query_row("SELECT COUNT(*) FROM goal_targets", [], |r| r.get(0))
        .unwrap();
    let bp: i64 = conn
        .query_row("SELECT COUNT(*) FROM planning_blueprints", [], |r| r.get(0))
        .unwrap();
    let cs: i64 = conn
        .query_row("SELECT COUNT(*) FROM ai_change_sets", [], |r| r.get(0))
        .unwrap();
...
fn test_t12_new_intent_escapes_planner() {
    ...
    assert_eq!(
        planning_continuation_decision(
            "1+1等于多少？只回答数字。",
            Some(WORKFLOW_STATE_CLARIFYING)
        ),
        PlanningContinuation::NewIntent
    );
...
fn test_t13_goaltarget_proposal_safety_pre_apply() {
    ...
    assert!(
        ops.len() >= 3,
        "GT create + GT activate + BP create：{ops:?}"
    );
    ...
    assert_eq!(
        ops[1].after.get("ref").and_then(|x| x.as_str()),
        Some("GT1")
    );
    assert_eq!(
        ops[1].after.get("status").and_then(|x| x.as_str()),
        Some("active")
    );
    ...
    assert_eq!(
        (gt, bp, tasks),
        (0, 0, 0),
        "未批准前 goal_targets/blueprints/tasks 0 变化"
    );
    ...
    GoalTargetRepository::new(&conn)
        .activate(p, existing.id)
        .unwrap();
    let ops2 = compile_to_changeset_ops(None, true, &draft);
    assert!(
        !ops2.iter().any(|o| o.entity_type == "goal_target"),
        "已有 active GoalTarget 时禁止 GT 提案 op"
    );
}
...
fn test_t14_apply_activates_target_and_blueprint() {
    ...
    let active = GoalTargetRepository::new(&conn)
        .list_active(p, None, None)
        .unwrap();
    ...
    let bp = PlanningRepository::new(&conn)
        .get_active(p)
        .unwrap()
        .expect("Apply 后 Blueprint active");
...
fn test_t15_existing_goaltarget_not_blocked_by_legacy_brief() {
    ...
    let instruction =
        build_planning_instruction(&truth.instruction, &PlanningWorkflowPayload::default());
