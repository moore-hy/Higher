...
fn setup(name: &str) -> (DbState, VaultState) {
    ...
    let vault_dir =
        std::env::temp_dir().join(format!("higher_dev0066r2_{}_{}", name, std::process::id()));
    (
...
fn r201_skip_projection_zero_side_effects() {
    ...
    v1["structured_json"] =
        json!({ "future_tasks": [ { "title": "旧投影任务", "planned_date": "2026-12-01" } ] });
    ...
    v2["structured_json"] =
        json!({ "future_tasks": [ { "title": "新蓝图任务A", "planned_date": "2026-11-01" } ] });
    ...
    assert!(
        archived.is_none(),
        "archived_at 必须仍为 NULL（R2-01：归档副作用关闭）：{archived:?}"
    );
    assert_eq!(est, 45);
    // 零新任务
...
fn r202_duplicate_keys_rejected() {
    ...
    assert_eq!(
        ...
        "重复 phase_key 必须拒绝：{out}"
    );
    assert!(
        out["message"].as_str().unwrap_or("").contains("重复"),
        "{out}"
    );
    ...
    assert_eq!(
        out2["status"], "invalid_action",
        "重复 milestone_key 必须拒绝：{out2}"
    );
    ...
    assert_eq!(
        out3["status"], "invalid_action",
        "悬空 phase_key 必须拒绝：{out3}"
    );
    assert!(
        out3["message"]
            .as_str()
...
fn r204_structured_json_idempotency() {
    ...
    let mut v1 = blueprint_action();
    v1["structured_json"] =
        json!({ "future_tasks": [ { "title": "A", "planned_date": "2026-12-01" } ], "note": "x" });
    let out1 = run_pack(&conn, &vault, p, c, "v1", &[v1.clone()]);
    assert_eq!(out1["status"], "applied", "{out1}");
    // key 顺序不同、内容相同 → no-op
    let mut same = blueprint_action();
    same["structured_json"] =
        json!({ "note": "x", "future_tasks": [ { "planned_date": "2026-12-01", "title": "A" } ] });
    ...
    assert_eq!(
        out3["status"], "applied",
        "structured_json 变化必须产生新版本：{out3}"
    );
...
fn r205_stale_undo_protection() {
    ...
    ChangeSetRepository::new(&conn)
        .undo(b["change_set_id"].as_i64().unwrap(), p)
        .unwrap();
    ...
    let active: String = conn.query_row(
        ...
        params![p], |r| r.get(0)).unwrap();
    assert!(
        active.contains("杭州电子科技大学"),
        "v3 仍 active：{active}"
    );
    // 最新 v3 可正常 Undo → v2 恢复 active
    ChangeSetRepository::new(&conn)
        .undo(v3["change_set_id"].as_i64().unwrap(), p)
        .unwrap();
    let restored: String = conn.query_row(
        "SELECT title FROM goal_targets WHERE profile_id=?1 AND role='safety' AND status='active'",
        params![p], |r| r.get(0)).unwrap();
    assert!(
        restored.contains("成都电子科技大学"),
        "最新版本 Undo 还原 v2：{restored}"
    );
    ...
    assert!(
        undo_bp2.is_err(),
        "Undo BP v2 必须拒绝（stale）：{undo_bp2:?}"
    );
    let active_title: String = conn
        .query_row(
    ...
    // 最新 v3 可正常 Undo → v2 恢复 active
    ChangeSetRepository::new(&conn)
        .undo(bp3["change_set_id"].as_i64().unwrap(), p)
        .unwrap();
