...
fn boot_tc001_db_ready_before_webview() {
    ...
    for tag in [
        ...
        assert!(
            lib.contains(&format!("[ANDROID-BOOT] {tag}")),
            "BOOT-TC001: 缺 {tag}"
        );
    }
    let ctx = web("contexts/ActiveProfileContext.tsx");
    assert!(
        ctx.contains("[ANDROID-BOOT] PROFILE_REQUEST")
            && ctx.contains("[ANDROID-BOOT] PROFILE_READY"),
        "BOOT-TC001: 前端 PROFILE 日志"
    );
    let layout = web("mobile/MobileLayout.tsx");
    assert!(
        layout.contains("[ANDROID-BOOT] FIRST_PAGE_READY"),
        "BOOT-TC001: FIRST_PAGE_READY"
    );
}
...
fn boot_tc002_bootstrap_timeout() {
    let ctx = web("contexts/ActiveProfileContext.tsx");
    assert!(
        ctx.contains("withTimeout") && ctx.contains("BOOT_TIMEOUT_MS"),
        "BOOT-TC002: 引导请求限时"
    );
}
...
fn boot_tc003_finite_retry() {
    let ctx = web("contexts/ActiveProfileContext.tsx");
    assert!(
        ctx.contains("BOOT_MAX_ATTEMPTS") && ctx.contains("BOOT_RETRY_DELAY_MS"),
        "BOOT-TC003: 有限重试"
    );
    assert!(
        ctx.contains("attempt < BOOT_MAX_ATTEMPTS"),
        "BOOT-TC003: 不无限循环"
    );
}
...
fn boot_tc004_error_screen() {
    ...
    assert!(
        ctx.contains("setGate({ phase: \"error\" })"),
        "BOOT-TC004: 初始化失败 → error（非 no_profiles）"
    );
    let app = web("App.tsx");
    assert!(
        app.contains("profile-gate--error") && app.contains("重新尝试"),
...
fn boot_tc005_retry_success() {
    let ctx = web("contexts/ActiveProfileContext.tsx");
    assert!(
        ctx.contains("retryBoot") && ctx.contains("setBootAttempt((a) => a + 1)"),
        "BOOT-TC005: retryBoot 重触发引导 effect"
    );
    // 引导成功路径保持原相位语义
    assert!(
        ctx.contains("phase: \"active\"")
            && ctx.contains("phase: \"no_profiles\"")
            && ctx.contains("phase: \"select\""),
        "BOOT-TC005: 成功路径相位不变"
    );
    let app = web("App.tsx");
    assert!(
        app.contains("retryBoot"),
        "BOOT-TC005: 错误屏接入 retryBoot"
    );
}
