...
fn test_planning_gate_three_states() {
    ...
    for m in [
        ...
        assert_eq!(
            planning_gate(m, true),
            PlanningGate::Planning,
            "assistant 写意图应进 Planning：{m}"
        );
    }
...
fn test_scenario_b_draft_valid_changeset_zero_writes_before_apply() {
    ...
    let items: i64 = conn
        ...
        .unwrap();
    assert_eq!(
        (tasks, day_goals, items),
        (0, 0, 0),
        "未 Apply 前正式数据必须 0 变化"
    );
    let _ = cs;
}
...
fn test_scenario_cdef_apply_then_queryable_everywhere() {
    ...
    assert!(
        titles.iter().any(|t| t.contains("极限计算基础题")),
        "Today 任务具体化：{titles:?}"
    );
...
fn test_scenario_g_reject_zero_writes() {
    ...
    assert!(
        ChangeSetRepository::new(&conn).apply(cs, p, false).is_err(),
        "rejected 状态不得 Apply"
    );
}
...
fn test_scenario_h_selective_apply() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        .create(p, None, None, "t", "s", &ops)
        .unwrap();
    let listed = ChangeSetRepository::new(&conn)
...
fn test_scenario_i_apply_conflict_rollback() {
    ...
    grepo
        ...
        .unwrap();
    let err = ChangeSetRepository::new(&conn)
        .apply(cs, p, false)
        .err()
        .unwrap_or_default();
...
fn test_scenario_o_rest_day_no_task() {
    ...
    let v = validate_plan_draft(&conn, p, &draft);
    assert!(
        v.errors.iter().any(|e| e.contains("休息日")),
        "rest_day 任务必须报错：{:?}",
        v.errors
    );
}
...
fn test_scenario_q_duplicate_guard() {
    ...
    assert!(
        v1.errors.iter().any(|e| e.contains("重复任务")),
        "draft 内重复：{:?}",
        v1.errors
    );
...
fn test_scenario_r_active_session_not_evidence() {
    ...
    let report = app_lib::ai::context_builder::build(
        ...
        p,
        "我现在学得怎么样",
        &app_lib::ai::context_builder::PageContext {
            page_label: "今日".to_string(),
            knowledge_path: None,
            session_title: None,
            date: None,
        ...
        "readonly",
        app_lib::ai::context_builder::ContextPurpose::Personal,
    )
    .unwrap();
    let l1 = &report.layers[0].text;
    assert!(l1.contains("当前有学习进行中"), "L1 应包含进行中状态：{l1}");
    assert!(
        l1.contains("进行中时长不算已完成学习量"),
        "必须声明不算完成证据"
    );
...
fn test_apply_summary_backend_driven_counts() {
    ...
    assert_eq!(
        written as usize,
        ops.len(),
        "真实写入数 = ops 数（Backend-driven 计数来源）"
    );
}
...
fn test_plan_does_not_add_actual_time() {
    ...
    assert_eq!(
        (total, days),
        (0, 0),
        "未来计划不得增加 actual learning（只有真实 Session 影响）"
    );
}
...
fn test_existing_knowledge_reuse_ok() {
    ...
    assert!(
        !v.errors.iter().any(|e| e.contains("知识")),
        "知识同名不报错：{:?}",
        v.errors
    );
}
...
fn test_planner_summary_template_nonzero_lines() {
    ...
    if k > 0 {
        lines.push(format!("新增 {} 个知识节点", k));
    }
    if t > 0 {
        lines.push(format!("安排 {} 个学习任务", t));
    }
    if rest_count > 0 {
...
fn test_ops_carry_period_date_fields() {
    ...
    let day_op = ops
        ...
        .find(|o| {
            o.entity_type == "goal"
                && o.after.get("goal_level").and_then(|x| x.as_str()) == Some("day")
        })
        .expect("day goal op");
    assert!(day_op
        .after
    ...
    let task_op = ops
        .iter()
        .find(|o| o.entity_type == "task")
        .expect("task op");
    assert!(
        task_op.after.get("planned_date").is_some(),
        "task after 必须带 planned_date（前端按日分组依据）"
    );
    let rest_op = ops
        .iter()
        .find(|o| o.after.get("day_kind") == Some(&json!("rest")));
    assert!(
        rest_op.is_some(),
        "day_kind=rest 必须落 after（前端「休息日」标签依据）"
    );
}
