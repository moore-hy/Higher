...
fn test_t1_planner_truth_goaltarget_is_primary_source() {
    ...
    conn.execute(
        "UPDATE study_profiles SET target_description='清华', target_date='2027-12-25' WHERE id=?1",
        params![p],
    )
    .unwrap();
    conn.execute(
    ...
    assert!(
        ctx.has_active_goal_target,
        "有 active GoalTarget 时主源存在"
    );
    let reach_title = ctx.reach_title.unwrap_or_default();
    assert!(
        reach_title.contains("华中科技"),
        "REACH 应为华科：{reach_title}"
    );
    assert!(
        ctx.instruction.contains("华中科技"),
        "instruction 必须以华科为正式目标"
    );
    assert!(
        ctx.instruction.contains("Active GoalTargets"),
        "instruction 含目标区块"
    );
    assert!(
        !ctx.instruction.is_empty(),
        "不得只写'请基于我的资料'而无正文"
    );
    let _ = today;
}
...
fn test_t2_planning_source_content_readable() {
    ...
    let sid = src_repo
        ...
        .unwrap();
    src_repo
        .store_chunks(sid, p, "数学一轮 2027-02-28 完成；之后进入强化阶段。")
        .unwrap();
    src_repo.set_status(sid, "ready").unwrap();

    let ctx = build_planning_truth_context(&conn, p);
    assert!(
        ctx.instruction.contains("数学计划.txt"),
        "truth context 应列出规划资料：{}",
        ctx.instruction
    );
    let text = src_repo.joined_text(p, sid).unwrap();
    assert!(
        text.contains("数学一轮"),
        "read_planning_source 数据通道必须能读到资料内容"
    );
}
...
fn test_t3_blueprint_task_user_modified_protected() {
    ...
    TaskRepository::new(&conn)
        ...
        .unwrap();
    let um: Option<String> = conn
        .query_row(
            "SELECT user_modified_at FROM tasks WHERE id=?1",
            params![bp_task_id],
            |r| r.get(0),
        )
        .unwrap();
    assert!(
        um.is_some(),
        "蓝图任务被正常 update_v2 修改后必须写 user_modified_at"
    );

    // 新蓝图激活/重投影 → 手改任务不得被归档
    let bp2 = repo
        .create_blueprint(p, "generic", "计划B", "# B", Some("{}"), "{}", "{}", 14)
        .unwrap();
    repo.activate(p, bp2.id, &today, 14).unwrap();
    let alive: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tasks WHERE id=?1 AND archived_at IS NULL",
            params![bp_task_id],
            |r| r.get(0),
        )
        .unwrap();
...
fn test_t4_review_no_change_completes() {
    ...
    let repo = PlanningRepository::new(&conn);
    let bp = repo
        .create_blueprint(p, "generic", "计划A", "# A", Some("{}"), "{}", "{}", 14)
        .unwrap();
    repo.activate(p, bp.id, &today, 14).unwrap();
    let old_next: Option<String> = conn
        .query_row(
            "SELECT next_review_at FROM planning_blueprints WHERE id=?1",
            params![bp.id],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let rid = rrepo
        .create_due(
            p,
            Some(bp.id),
            &add_days(&conn, &today, -14),
            &today,
            "manual",
        )
        .unwrap();
    let snapshot = PlanningReviewRepository::build_snapshot(
        &conn,
        p,
        Some(bp.id),
        &add_days(&conn, &today, -14),
    ...
    let new_next: Option<String> = conn
        .query_row(
            "SELECT next_review_at FROM planning_blueprints WHERE id=?1",
            params![bp.id],
            |r| r.get(0),
...
fn test_t5_review_change_blueprint_v2_chain() {
    ...
    let repo = PlanningRepository::new(&conn);
    let bp1 = repo
        .create_blueprint(p, "generic", "计划V1", "# V1", Some("{}"), "{}", "{}", 14)
        .unwrap();
    ...
    let cs_id = rev
        .change_set_id
        .expect("ADJUSTMENT_PROPOSAL 必须创建 ChangeSet");
    ...
    assert!(
        rev2.resulting_blueprint_id.is_some(),
        "应回填 resulting_blueprint_id"
    );
...
fn test_t7_personal_profile_version_snapshot_immutable() {
    ...
    // v1：A+B
    prepo
        .save_draft_with_sources(p, "# 档案 v1", Some("{}"), &[a, b])
        .unwrap();
    ...
    prepo
        .save_draft_with_sources(p, "# 档案 v2", Some("{}"), &[a, b, c])
        .unwrap();
    prepo.confirm(p).unwrap();
...
fn test_t8_personal_export_sources_are_personal_not_planning() {
    ...
    PlanningSourceRepository::new(&conn)
        .insert(
            p,
            "user_file",
            "规划资料.txt",
    ...
    let ps = prepo
        ...
        .unwrap();
    prepo
        .save_draft_with_sources(p, "# 档案", Some("{}"), &[ps])
        .unwrap();
    prepo.confirm(p).unwrap();
    ...
    assert!(
        !srcs.iter().any(|s| s.file_name.contains("规划资料")),
        "Personal 导出不得混入 Planning Sources"
    );
}
...
fn test_t9_personal_xlsx_import_extracts_and_saves() {
    ...
    assert!(
        names.contains(&"xl/workbook.xml".to_string()),
        "zip entries: {names:?}"
    );
    let wb = entries
        .iter()
...
fn test_t10_manual_planning_without_ai() {
    ...
    let bp = repo
        ...
        .unwrap();
    let ph = repo
        .add_phase(bp.id, "P1", "基础期", Some(&today), None, "打基础", 1)
        .unwrap();
    repo.add_milestone(
        bp.id,
        Some(ph),
    ...
    assert!(
        GoalTargetRepository::new(&conn)
            .list_active(p, Some("generic"), None)
            .unwrap()
            .len()
            >= 1
    );
...
fn make_min_xlsx(text: &str) -> Vec<u8> {
    ...
    let entries: Vec<(&str, &str)> = vec![
        (
            "xl/workbook.xml",
            r#"<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Sheet1" r:id="rId1"></sheet></sheets></workbook>"#,
        ),
        (
            "xl/_rels/workbook.xml.rels",
            r#"<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"></Relationship></Relationships>"#,
        ),
