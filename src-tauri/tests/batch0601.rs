...
use app_lib::ai::action::{
    compile_action, resolve_recurring_rule, resolve_task, validate_action, EntityHint, Resolution,
    RuleUpdatePayload, SemanticAction, TemporalIntentSerde,
};
use app_lib::ai::planner::{
    ...
    PlannerQuestion, PlanningContinuation, PlanningWorkflowPayload, ToolRoundOutcome,
...
use app_lib::ai::skills::{
    registry, skill_by_id, validate_registry, ToolPermission, CAPABILITY_REGISTRY, TOOL_REGISTRY,
};
...
fn t2_legacy_rule_defaults() {
    ...
    let rule = RecurringRuleRepository::new(&conn)
        ...
        .unwrap();
    assert_eq!(
        rule.estimated_minutes, None,
        "T2: legacy estimated_minutes = null"
    );
    assert_eq!(
        rule.task_kind, "structured",
        "T2: legacy task_kind = structured"
    );
...
fn t3_migration_idempotent() {
    ...
    assert_eq!(
        n,
        app_lib::migrations::latest_version() as i64,
        "T3: migrations 只执行一次"
    );
}
...
fn t4_latest_schema_v023() {
    ...
    assert_eq!(v, 31, "T4: 最新 Schema = v031");
...
fn t5_t10_skill_contracts() {
    ...
    for s in registry() {
        ...
        assert!(
            ins.contains("Version"),
            "T10: {} 必须是 versioned contract",
            s.id
        );
        assert!(
            !ins.contains("src/") && !ins.contains(".rs"),
            "T10: {} 禁止引用源码路径",
            s.id
        );
        assert!(
            skill_by_id(s.id).is_some(),
            "skill_by_id({}) 必须命中",
            s.id
        );
    }
    ...
    assert!(
        caps.iter()
            .all(|c| !c.contains("direct_write") && !c.contains(".write")),
...
fn t11_t14_temporal_resolve() {
    ...
    assert_eq!(
        TemporalIntent::WeekdayRelative { weekday: 1 }
            .resolve(&e)
            .unwrap(),
        "2026-08-24"
    );
    assert_eq!(
        TemporalIntent::WeekdayRelative { weekday: 5 }
            .resolve(&e)
            .unwrap(),
        "2026-08-28"
    );
    ...
    assert_eq!(TemporalIntent::Yesterday.resolve(&e).unwrap(), "2026-08-20");
    assert!(TemporalIntent::OffsetDays { days: 366 }
        .resolve(&e)
        .is_err());
    assert!(TemporalIntent::OffsetDays { days: -366 }
        .resolve(&e)
    ...
    assert!(AiRuntimeEnvelope::validated(
        "2026-8-21",
        "2026-08-21 10:00",
        480,
        "p",
    ...
    .is_err());
    // weekday 由后端推导（不信调用方）
    let e2 = AiRuntimeEnvelope::validated(
        "2026-08-21",
        "2026-08-21 10:00",
        480,
        "p",
    ...
    let e3 = AiRuntimeEnvelope::validated(
        "2026-08-21",
        "2026-08-21 10:00",
        480,
        "p",
    ...
    .unwrap();
    assert!(
        e3.prompt_block().contains("2026-08-18"),
        "page_date 与 runtime date 必须区分"
    );
}
...
fn t17_daily_rule_plus_initial_task() {
    ...
    assert_eq!(
        compiled.ops.len(),
        2,
        "T17: daily today → rule create + initial task"
    );
    ...
    assert_eq!(
        t.after["recurring_rule_ref"], "R1",
        "T17: initial task 引用 R1"
    );
    assert_eq!(
        t.after["planned_date"], "2026-08-21",
        "T17: planned_date = runtime today"
    );
...
fn t20_t21_materialize_idempotent() {
    ...
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-08-21").unwrap(),
        0,
        "T20: 幂等"
    );
    ...
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-10-30").unwrap(),
        1
    );
}
...
fn t22_knowledge_optional() {
    ...
    assert!(
        compiled
            .ops
            .iter()
            .all(|o| o.after.get("learning_item_id").is_none()),
    ...
    assert!(
        compile_action(&conn, p, &e, &bad).is_err(),
        "weekly 至少一天"
    );
}
...
fn t23_rich_rule_inheritance() {
    ...
    RecurringRuleRepository::new(&conn)
        ...
        .unwrap();
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-08-22").unwrap(),
        1
    );
...
fn t24_weekly_only_matching_days() {
    ...
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-08-23").unwrap(),
        0,
        "周日不命中"
    );
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-08-24").unwrap(),
        1,
        "周一命中"
    );
...
fn t25_t27_resolver() {
    ...
    // T26：0 匹配 → NotFound
    let miss = EntityHint {
        title_hint: "量子物理".into(),
        date: None,
        ..Default::default()
    };
    assert!(
        matches!(
            resolve_task(&conn, p, &miss, &e).unwrap(),
            Resolution::NotFound(_)
        ),
        "T26: 0 匹配 NotFound"
    );
    // T27：2+ 匹配 → Ambiguous（禁止自动选）
    repo.create_for_profile(
        p,
        None,
        "背单词（复习）",
        Some("2026-08-21"),
    ...
    let rhint = EntityHint {
        title_hint: "背单词".into(),
        date: None,
        ..Default::default()
    };
    assert!(matches!(
        resolve_recurring_rule(&conn, p, &rhint).unwrap(),
        Resolution::NotFound(_)
    ));
}
...
fn t28_t29_task_update_changeset() {
    ...
    assert_eq!(
        date.as_deref(),
        Some("2026-08-21"),
        "T29: 未提供 date 保持 before"
    );
    assert_eq!(
        time.as_deref(),
        Some("09:00"),
        "T29: 未提供 time 保持 before"
    );
    // Resolver 仍可解析（before 快照兼容）
    let hint = EntityHint {
        title_hint: "错题复盘".into(),
        date: Some(TemporalIntentSerde(TemporalIntent::Today)),
        ..Default::default()
    };
    assert!(matches!(
        resolve_task(&conn, p, &hint, &e).unwrap(),
...
fn t30_t31_recurring_update_and_disable() {
    ...
    let rule = RecurringRuleRepository::new(&conn)
        ...
        .unwrap();
    assert_eq!(
        materialize_recurring_tasks(&conn, p, "2026-08-20").unwrap(),
        1
    );
    ...
    assert_eq!(
        compiled.ops[0].entity_type, "recurring_rule",
        "T30: 只生成 rule update"
    );
    let cs = ChangeSetRepository::new(&conn)
        .create(
    ...
    let old_task_title: String = conn
        .query_row(
            "SELECT title FROM tasks WHERE planned_date='2026-08-20'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(
        old_task_title, "每天背单词",
        "T30: 过去已生成 Task 保持不变"
    );
...
fn t32_t33_fast_chat_provider_profile() {
    ...
    assert_eq!(
        tool_definitions_for_scopes(&scopes_for_route("fast_chat")),
        json!([])
    );
    ...
    assert_eq!(
        trace.secondary_requests, 0,
        "T33: FastChat Memory Extract = 0"
    );
...
fn t34_fast_chat_private_context() {
    ...
    assert!(
        bound
            .iter()
            .all(|(_, r, _)| r == "user" || r == "assistant"),
        "T34: 只保留对话轮次"
    );
    assert_eq!(bound.len(), 3);
    let joined: String = bound.iter().map(|(_, _, c)| c.clone()).collect::<String>();
    assert!(
        !joined.contains("PersonalProfile") && !joined.contains("Memory"),
        "T34: 私有上下文不进入 FastChat"
    );
    // 当前消息 id 被排除（永不重复）
    let bound2 = bound_history(&history, 2, 8, 14_000);
    assert!(
        !bound2.iter().any(|(id, _, _)| *id == 2),
        "当前消息按 id 排除"
    );
    // 轮数预算：最多 8 轮 = 16 条
    let long: Vec<(i64, String, String)> = (0..40)
        ...
        .collect();
    assert_eq!(
        bound_history(&long, 999, 8, 1_000_000).len(),
        16,
        "8 轮预算"
    );
    // 字符预算：超预算丢最旧
...
fn t35_t39_semantic_action_call_profile() {
    ...
    assert_eq!(
        TOOL_ALLOWLIST.len(),
        24,
        "全量 allowlist 为 24（0066 Phase B 读取路径）"
    );
    assert_eq!(
    ...
    let prompt = semantic_action_prompt(
        "帮我创建今天背单词任务",
        &e,
        &["task".to_string(), "time".to_string()],
    );
    assert!(
        prompt.contains("Runtime Time Truth"),
        "T36: 必须含 Runtime Envelope"
    );
    assert!(
        prompt.contains("Skill: task") && prompt.contains("Skill: time"),
        "T36: 注入 selected skills 全文"
    );
    assert!(
        !prompt.contains("Skill: recurring_task"),
        "T36: 未选 skill 不注入"
    );
    assert!(
        !prompt.contains("list_planning_sources") || !prompt.contains("get_profile_summary"),
        "T36: 不含无关工具"
    );
    // T37：Action success 不需要第二次模型调用（compiler 直接产出确定性 summary）
    let compiled = compile_action(&conn, p, &e, &create_task_action()).unwrap();
    assert!(
        !compiled.summary.trim().is_empty() && !compiled.title.trim().is_empty(),
        "T37: 确定性总结由 Compiler 产出"
    );
    // T38：Invalid semantic JSON → None；Repair Once 后仍非法 → None（最多一次修复机会）
    let bad1 = "这不是 JSON";
    let bad2 = "```json\n{\"type\":\"create_task\",\"title\":\"x\"}\n```"; // 缺 date → schema 失败
    assert!(
        parse_semantic_action(bad1).is_none(),
        "T38: 非法输入解析失败"
    );
    assert!(
        parse_semantic_action(bad2).is_none(),
        "T38: schema 缺字段解析失败"
    );
    ...
    assert_eq!(
        count(&conn, "tasks") + count(&conn, "recurring_task_rules"),
        before,
        "T39: 0 mutation"
    );
    // 合法输出（含 ```json 围栏）可解析
...
fn t40_t42_tool_registry() {
    ...
    for t in TOOL_ALLOWLIST {
        assert!(
            reg.contains(t),
            "T41: allowlist 工具 {t} 必须在 Registry（带 permission）"
        );
    }
...
fn t44_t45_planning_scoping() {
    ...
    assert!(
        names.len() <= 6,
        "T44: Planning 只带 planning+web（≤6），当前 {names:?}"
    );
    for banned in [
        "get_profile_summary",
        "list_knowledge_tree",
    ...
    for (t, args) in cases {
        ...
        assert!(
            serde_json::from_str::<serde_json::Value>(&out.unwrap()).is_ok(),
            "{t} 返回合法 JSON"
        );
    }
}
...
fn t46_t47_message_assembly_invariants() {
    ...
    let user_cnt = msgs2
        .iter()
        .filter(|m| m.role == "user" && m.content.contains("华中科技大学"))
        .count();
    assert_eq!(user_cnt, 2, "T47: 历史同文消息保留 + 当前消息（共 2）");
    // 当前消息 id 命中历史 → 被排除
    let msgs3 = build_chat_messages(
        "SYS",
        "CTX",
        "INS",
        &dup_hist,
    ...
    assert_eq!(
        msgs3.iter().filter(|m| m.role == "user").count(),
        1,
        "按 id 排除当前消息"
    );
}
...
fn t48_t49_goaltarget_canonical() {
    ...
    assert!(
        truth.has_active_goal_target,
        "T48: active GoalTarget 为主源"
    );
    ...
    assert!(
        !truth2.has_active_goal_target,
        "T49: legacy Goal 不晋升为正式目标"
    );
}
...
fn t50_t52_continuation_decisions() {
    ...
    let mut payload = PlanningWorkflowPayload {
        ...
        pending_questions: vec![
            PlannerQuestion {
                key: "institution_name".into(),
                question: "目标院校？".into(),
            },
            PlannerQuestion {
                key: "program_name".into(),
                question: "目标专业？".into(),
            },
        ],
        ..Default::default()
...
fn t53_goaltarget_proposal_approval_first() {
    ...
    let cs = ChangeSetRepository::new(&conn)
        ...
        .unwrap();
    assert_eq!(
        count(&conn, "goal_targets"),
        0,
        "T53: Apply 前 0 落库（Approval First）"
    );
    ChangeSetRepository::new(&conn).apply(cs, p, false).unwrap();
    assert_eq!(count(&conn, "goal_targets"), 1, "T53: Apply 后创建");
    let truth = build_planning_truth_context(&conn, p);
    assert!(
        truth.has_active_goal_target || count(&conn, "goal_targets") == 1,
        "提案落库后进入正式目标体系"
    );
}
...
fn t54_existing_goaltarget_not_blocked() {
    ...
    let gt = GoalTargetRepository::new(&conn)
        .create(
            ...
            "浙江大学 软件",
            None,
            r#"{"institution_name":"浙江大学","program_name":"软件工程"}"#,
            "{}",
            "candidate",
        )
        .unwrap();
    ...
    assert!(
        truth.has_active_goal_target,
        "T54: GoalTarget 存在 → 不被 legacy Brief 阻塞"
    );
...
fn t55_no_second_main_completion() {
    ...
    assert!(matches!(
        classify_tool_round(Some(&calls), None),
        ToolRoundOutcome::ExecuteTools(_)
    ));
}
...
fn t56_planner_answer_routes_to_continuation() {
    ...
    assert!(
        prompt.contains("进行中"),
        "T56: Router 必须看到 active planner"
    );
    assert!(
        prompt.contains("你的目标院校"),
        "T56: Router 必须看到等待中的问题"
    );
...
fn t57_task_request_escapes_to_semantic_action() {
    ...
    // ① 不是显式取消
    assert!(
        !app_lib::ai::planner::is_workflow_exit_intent(msg),
        "T57: 非取消"
    );
    ...
    let compiled = compile_action(&conn, p, &e, &act).unwrap();
    assert_eq!(
        compiled.ops[1].after["planned_date"], "2026-08-21",
        "T57: 日期 = runtime today"
    );
}
