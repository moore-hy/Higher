...
export function changedKinds(p) {
    const kinds = [];
    if (p.profiles_changed > 0)
        kinds.push("profiles");
    if (p.goals_changed > 0)
        kinds.push("goals");
    if (p.learning_items_changed > 0)
        kinds.push("learningItems");
    if (p.tasks_changed > 0)
        kinds.push("tasks");
    return kinds;
}
export function shouldRefresh(p, kind) {
 ...
 */
export function createSyncRefreshDispatcher(handlers) {
    return (p) => {
        let fired = 0;
